...
if QWidget is not None:
    ...
    class QpcrMixCalculatorWidget(QWidget):
        ...
        def _build_ui(self) -> None:
            ...
            self._reactions.setObjectName("qpcrMixReactionsField")
            self._reaction_volume = _line_edit("例如 20")
            self._reaction_volume.setObjectName("qpcrMixReactionVolumeField")
            self._master_mix_value = _line_edit("例如 10 或 50")
            self._master_mix_value.setObjectName("qpcrMixMasterMixValueField")
            self._master_mix_mode = _combo(("体积（µL）", "比例（%）"), "体积（µL）")
            self._master_mix_mode.setObjectName("qpcrMixMasterMixModeCombo")
            self._forward = _line_edit("例如 0.4")
            self._forward.setObjectName("qpcrMixForwardPrimerField")
            self._reverse = _line_edit("例如 0.4")
            self._reverse.setObjectName("qpcrMixReversePrimerField")
            self._template = _line_edit("例如 2")
            self._template.setObjectName("qpcrMixTemplateField")
            self._loss_percent = _line_edit("默认 10")
            self._loss_percent.setObjectName("qpcrMixOveragePercentField")
            self._loss_percent.setText("10")
            button = QPushButton("计算配液用量")
            button.setObjectName("qpcrMixCalculateButton")
            button.setProperty("buttonBehavior", "calculates_qpcr_mix_plan")
            ...
            self._result = ResultPanel()
            self._result.setObjectName("qpcrMixResultPanel")
            root.addWidget(self._result)
            self._copy_button = _copy_button_for(self._result)
            self._copy_button.setObjectName("qpcrMixCopyResultButton")
            self._copy_button.setProperty("buttonBehavior", "copies_qpcr_mix_result_after_calculation")
            self._copy_button.setProperty("disabledReason", "Requires a valid qPCR mix calculation before copy is enabled.")
            self._copy_button.setProperty("formalActionEnabled", False)
            root.addWidget(self._copy_button)
            root.addStretch(1)
    ...
    class ReagentPreparationWorkflowWidget(QWidget):
        def __init__(self, store: ReagentTemplateStore | None = None) -> None:
            ...
            root.setContentsMargins(0, 0, 0, 0)
            scroll = QScrollArea()
            scroll.setObjectName("labToolsReagentPreparationScroll")
            scroll.setWidgetResizable(True)
            content = QWidget()
            content.setObjectName("labToolsReagentPreparationContent")
            content_layout = QVBoxLayout(content)
            content_layout.setContentsMargins(SPACING["lg"], SPACING["lg"], SPACING["lg"], SPACING["lg"])
            content_layout.setSpacing(SPACING["lg"])
            content_layout.addWidget(ReagentTemplateManagerWidget(store))
            content_layout.addWidget(ReagentPreparationWidget(store))
