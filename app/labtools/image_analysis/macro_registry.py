...
BUILTIN_MACRO_TEMPLATES: tuple[MacroTemplate, ...] = (
    ...
    _template(
        ...
        "通用颗粒/细胞识别 ImageJ Macro",
        "cell_experiment",
        "generic_particle_analysis",
        "cell_experiment/generic_particle_analysis_basic.ijm",
        parameter_schema={
            "threshold_method": "str",
            "particle_polarity": "dark/bright",
            "background_rolling_px": "number",
            "blur_sigma": "number",
            "min_particle_area_px": "int",
            "max_particle_area_px": "int/Infinity",
            "min_circularity": "number",
            "max_circularity": "number",
            "watershed": "bool",
        },
        version="0.3-imagej-workflow",
        description="真实 ImageJ macro 生成模板：批量识别有清晰边界的颗粒/细胞信号；执行仍受外部引擎 gate 和人工复核控制。",
        expected_outputs=("outputs/generic_particle_results.csv", "logs/run_log.txt", "review/manual_review.json"),
    ),
    _template(
        "roi_intensity_batch_basic",
        "ROI 强度批量测量 ImageJ Macro",
        "cell_experiment",
        "roi_intensity_batch",
        "cell_experiment/roi_intensity_batch_basic.ijm",
        parameter_schema={"roi_zip_path": "path", "background_roi_index": "int", "measurement_channel": "int"},
        version="0.3-imagej-workflow-gated",
        description="ImageJ macro 生成模板：依赖 ROI zip 输入 gate；缺少 ROI zip 时只写入 missing_roi_zip blocker，不伪装正式结果。",
        expected_outputs=("outputs/roi_intensity_results.csv", "logs/run_log.txt", "review/manual_review.json"),
    ),
    _template(
        "migration_streak_roi_basic",
        "迁移/划痕 ROI ImageJ Macro",
        "cell_experiment",
        "migration_streak_roi",
        "cell_experiment/migration_streak_roi_basic.ijm",
        parameter_schema={
            "streak_threshold_method": "str",
            "streak_polarity": "dark/bright",
            "streak_blur_sigma": "number",
            "streak_min_area_px": "int",
            "signal_threshold_method": "str",
            "signal_polarity": "dark/bright",
            "signal_min_area_px": "int",
            "signal_max_area_px": "int",
        },
        version="0.3-imagej-workflow-review",
        description="ImageJ macro 生成模板：迁移/划痕 ROI 识别和残余面积估算；需要用真实样本继续阈值验证。",
        expected_outputs=("outputs/migration_streak_roi_results.csv", "logs/run_log.txt", "review/manual_review.json"),
    ),
    _template(
        ...
        "cell_experiment/cell_skeleton_morphology.ijm",
        parameter_schema={"threshold_method": "str", "foreground_polarity": "dark/bright", "blur_sigma": "number", "prune_method": "str", "save_skeleton_image": "bool"},
        minimum_engine_requirement=IMAGE_ANALYSIS_ENGINE_REQUIREMENT_FIJI,
        version="0.3-fiji-plugin-gated",
        description="Fiji macro 生成模板：依赖 Skeletonize/Analyze Skeleton 插件环境；未通过 Fiji gate 前不得声明正式可用。",
        expected_outputs=("outputs/cell_skeleton_morphology_results.csv", "logs/run_log.txt", "review/manual_review.json"),
    ),
    _template(
...
DEFAULT_MACRO_BY_ANALYSIS: dict[tuple[str, str], str] = {
    ...
    ("cell_experiment", "generic_particle_analysis"): "generic_particle_analysis_basic",
    ("cell_experiment", "roi_intensity_batch"): "roi_intensity_batch_basic",
    ("cell_experiment", "migration_streak_roi"): "migration_streak_roi_basic",
    ("cell_experiment", "cell_skeleton_morphology"): "cell_skeleton_morphology_fiji",
