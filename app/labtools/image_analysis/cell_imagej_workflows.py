...
CELL_IMAGEJ_EXPERIMENTS: tuple[CellImageJExperimentSpec, ...] = (
    ...
    CellImageJExperimentSpec(
        ...
        analysis_type="generic_particle_analysis",
        aliases=("particle_analysis", "particles", "particle", "通用颗粒", "颗粒识别", "细胞颗粒识别"),
        title="通用颗粒识别图片处理",
    ...
    CellImageJExperimentSpec(
        ...
        analysis_type="roi_intensity_batch",
        aliases=("roi_intensity", "measure_rois", "roi_measurement", "roi强度", "roi测量", "批量roi"),
        title="ROI 强度批量测量",
    ...
    CellImageJExperimentSpec(
        ...
        analysis_type="migration_streak_roi",
        aliases=("streak_roi", "migration_streak", "scratch_roi", "迁移划痕", "划痕roi", "迁移roi"),
    ...
    CellImageJExperimentSpec(
        ...
        analysis_type="cell_skeleton_morphology",
...
def _normalize_parameters(experiment: CellImageJExperimentSpec, parameters: Mapping[str, Any]) -> dict[str, str | int | float | bool]:
    ...
    aliases = {
        ...
        "背景扣除半径": "background_rolling_px",
        "最小圆度": "min_circularity",
        "最大圆度": "max_circularity",
        "是否 Watershed": "watershed",
        "是否分水岭": "watershed",
        "最小阳性面积": "min_positive_area_px",
        "ROI zip 路径": "roi_zip_path",
        "背景 ROI index": "background_roi_index",
        "测量通道": "measurement_channel",
        "骨架阈值模式": "threshold_method",
        "保存骨架图片": "save_skeleton_image",
    }
...
def _render_generic_particle_macro(input_dir: Path, output_dir: Path, parameters: Mapping[str, str | int | float | bool], result_csv_name: str) -> str:
    return _macro_header(input_dir, output_dir, result_csv_name, parameters) + """
...
def _render_roi_intensity_macro(input_dir: Path, output_dir: Path, parameters: Mapping[str, str | int | float | bool], result_csv_name: str) -> str:
    return _macro_header(input_dir, output_dir, result_csv_name, parameters) + """
roiZip = roi_zip_path;
if (roiZip == "")
...
def _render_migration_streak_roi_macro(input_dir: Path, output_dir: Path, parameters: Mapping[str, str | int | float | bool], result_csv_name: str) -> str:
    return _macro_header(input_dir, output_dir, result_csv_name, parameters) + """
...
def _render_cell_skeleton_macro(input_dir: Path, output_dir: Path, parameters: Mapping[str, str | int | float | bool], result_csv_name: str) -> str:
    return _macro_header(input_dir, output_dir, result_csv_name, parameters) + """
...
def _macro_header(input_dir: Path, output_dir: Path, result_csv_name: str, parameters: Mapping[str, str | int | float | bool]) -> str:
    lines = [
        "// Generated by BioMedPilot LabTools from dev/labtools@0bd04b2 + @f77bfe4 cell ImageJ workflows.",
