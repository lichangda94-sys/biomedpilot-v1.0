...
LABTOOLS_TOOLS: tuple[LabToolsTool, ...] = (
    ...
    LabToolsTool(
        ...
        requires_imagej_fiji=False,
        is_planned_only=False,
        description="Western Blot 上样体系计算器可用；ImageJ-assisted 条带定量 workflow 仍为 planned / 未启用；不启用 WB 图像分析、条带识别、灰度定量或自动 ROI。",
        boundary_statement="当前 L4 仅支持 WB 上样体系计算，不启用 WB 图像分析、条带识别、灰度定量、自动 ROI 或结果解释。",
        future_capabilities=(
            "WB 上样体系计算和横向 lane layout",
