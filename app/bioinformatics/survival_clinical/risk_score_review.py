...
RISK_SCORE_GUARD_COPY = (
    ...
    "No high/low-risk group, clinical nomogram interpretation, calibration-based clinical conclusion, decision recommendation, or report-ready package is generated."
)
...
def _risk_score_entry(entry: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not any(isinstance(item, dict) and item.get("artifact_type") == "risk_score_result_table" for item in artifacts):
        return None
    if entry.get("report_artifacts") or entry.get("report_ready_eligible") is True:
        return None
    return entry
...
def _disabled_downstream() -> dict[str, str]:
    return {
        ...
        "plot": "Risk score plot artifacts, when present, remain statistical visualization artifacts only and do not create risk groups or report-ready output.",
