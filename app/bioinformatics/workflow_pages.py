...
from app.bioinformatics.deg_task_plan import build_deg_preflight, load_deg_preflight_manifest, save_deg_task_plan
...
class BioinformaticsAnalysisTaskCenterWidget(QWidget):
    ...
    def __init__(
        ...
        *,
        on_continue: Callable[[Path], None] | None = None,
        on_back: Callable[[], None] | None = None,
        on_configure_deg: Callable[[], None] | None = None,
        on_view_imported_deg: Callable[[], None] | None = None,
        on_configure_immune_scoring: Callable[[], None] | None = None,
        parent: QWidget | None = None,
    ) -> None:
        super().__init__(parent)
        self._project_root: Path | None = None
        self._on_configure_deg = on_configure_deg
        self._on_view_imported_deg = on_view_imported_deg
        self._on_configure_immune_scoring = on_configure_immune_scoring
    ...
    def open_immune_scoring(self) -> dict[str, object] | None:
        if self._project_root is None:
            self._status_label.setText("请先创建或打开生信分析项目。")
            return None
        if self._on_configure_immune_scoring is not None:
            try:
                self._on_configure_immune_scoring(self._project_root)
            except TypeError:
                self._on_configure_immune_scoring()
    ...
    def _build_ui(self) -> None:
        ...
        immune_button = _button("免疫浸润 / TME评分", "secondaryButton", self.open_immune_scoring, role="secondary")
        immune_button.setObjectName("bioinformaticsImmuneScoringEntryButton")
        immune_button.setProperty("resultSemanticKey", "testing_summary_only")
        actions.addWidget(immune_button)
...
class BioinformaticsResultsBrowserWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        open_results = _button("打开结果文件夹 - gated", "secondaryButton", lambda: self._status_label.setText("结果文件夹打开已门控：暂无 formal result。"))
        open_results.setObjectName("bioinformaticsOpenResultFolderGatedButton")
        open_results.setProperty("formalActionEnabled", False)
        open_results.setProperty("resultSemanticKey", "preflight_only")
        open_results.setProperty("disabledState", "formal_result_missing")
        open_results.setEnabled(False)
        actions.addWidget(open_results)
        open_params = _button("打开参数 JSON - gated", "secondaryButton", lambda: self._status_label.setText("参数 JSON 打开已门控：暂无 formal result artifact。"))
        open_params.setObjectName("bioinformaticsOpenResultParamsGatedButton")
        open_params.setProperty("formalActionEnabled", False)
        open_params.setProperty("resultSemanticKey", "preflight_only")
        open_params.setProperty("disabledState", "formal_result_missing")
        open_params.setEnabled(False)
        actions.addWidget(open_params)
...
class BioinformaticsReportViewerWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        self._generate_report_button.setEnabled(False)
        actions.addWidget(self._generate_report_button)
        open_report = _button("打开报告文件 - gated", "secondaryButton", lambda: self._status_label.setText("报告文件打开已门控：report-ready gate 未通过。"))
        open_report.setObjectName("bioinformaticsOpenReportFileGatedButton")
        open_report.setProperty("formalActionEnabled", False)
        open_report.setProperty("reportReadyPackageAllowed", False)
        open_report.setProperty("disabledState", "report_not_ready")
        open_report.setEnabled(False)
        actions.addWidget(open_report)
        open_folder = _button("打开报告文件夹 - gated", "secondaryButton", lambda: self._status_label.setText("报告文件夹打开已门控：report-ready gate 未通过。"))
        open_folder.setObjectName("bioinformaticsOpenReportFolderGatedButton")
        open_folder.setProperty("formalActionEnabled", False)
        open_folder.setProperty("reportReadyPackageAllowed", False)
        open_folder.setProperty("disabledState", "report_not_ready")
