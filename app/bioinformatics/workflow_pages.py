...
class BioinformaticsAnalysisTaskCenterWidget(QWidget):
    ...
    def _build_ui(self) -> None:
        ...
        survival_layout.addWidget(_muted("B14 允许 single-variable Cox controlled MVP 和 forest plot spec；多因素 Cox、风险评分、临床结论和 survival report-ready 仍禁用。"))
        ...
        _fill_table(
            ...
            _analysis_ui_gate_rows(
                [
                    ...
                    *(analysis_state.get("analysis_architecture_gate_rows", []) or []),
                    *(analysis_state.get("module_interface_rows", []) or []),
                    *(analysis_state.get("module_mode_readiness_rows", []) or []),
                    *(analysis_state.get("environment_artifact_rows", []) or []),
                    *(analysis_state.get("resource_artifact_rows", []) or []),
                    ...
                    *(analysis_state.get("full_activation_module_rows", []) or []),
                    *(analysis_state.get("standard_worker_migration_rows", []) or []),
                    *(analysis_state.get("analysis_architecture_remediation_rows", []) or []),
                    *(analysis_state.get("analysis_environment_gate_rows", []) or []),
                    *(analysis_state.get("analysis_resource_gate_rows", []) or []),
                ]
            ),
        )
    ...
    def _render_formal_deg_review(self, review: dict[str, object]) -> None:
        ...
        _fill_table(
            ...
            [
                ...
                ["standard_result_package", provenance.get("standard_result_package", "")],
                ["standard_package_validation", provenance.get("standard_package_validation_status", "")],
                ["standard_package_table_path", provenance.get("standard_package_table_path", "")],
                ["standard_package_source_policy", provenance.get("standard_package_source_policy", "")],
                ["worker_boundary", provenance.get("worker_boundary_type", "")],
                ["worker_migration_status", provenance.get("worker_migration_status", "")],
