...
class SurvivalPreflightResult:
    success: bool
    project_id: str
    source_path: str
    dataset_count: int
    ready_for_survival_count: int
    output_path: str
    message: str
    error_count: int = 0
...
class SurvivalService:
    def __init__(
        self,
        *,
        adapter: SurvivalAdapter | None = None,
        task_center: TaskCenter | None = None,
        data_center: DataCenter | None = None,
        storage_root: Path | None = None,
    ) -> None:
        self._adapter = adapter or SurvivalAdapter()
    ...
    def create_preflight(self, *, project_id: str, cleaning_plan_path: str) -> SurvivalPreflightResult:
        ...
        if validation_error is not None:
            result = SurvivalPreflightResult(
                success=False,
                project_id=project_id,
                source_path=cleaning_plan_path,
                dataset_count=0,
                ready_for_survival_count=0,
                output_path="",
        ...
        try:
            ...
            if "cleaning_items" not in payload:
                raise ValueError("生存分析预检需要数据清洗计划。")
            items = self._adapter.build_preflight(payload)
            ready_count = sum(1 for item in items if item.status == "ready_for_survival_setup")
            ...
            result = SurvivalPreflightResult(
                ...
                ready_for_survival_count=ready_count,
                output_path=str(output_path),
                message=f"生存分析预检已生成：{ready_count}/{len(items)} 个数据集具备设置前置条件。",
                details={"survival_analysis_executed": False, "network_used": False},
            )
            self._data_center.register_asset(
                project_id=project_id,
                module="bioinformatics",
                data_type="geo_survival_preflight",
                source_path=str(source_path),
                output_path=str(output_path),
                status="available",
            )
            ...
            result = SurvivalPreflightResult(
                ...
                ready_for_survival_count=0,
                output_path="",
                message="生存分析预检失败，请确认输入来自数据清洗计划。",
                error_count=1,
    ...
    def _start_task(self, *, project_id: str, source_path: str) -> TaskRecord:
        ...
        return self._task_center.register_task(
            ...
            module="bioinformatics",
            title="Survival Preflight",
            project_id=project_id,
            status=TaskStatus.RUNNING,
            started_at=now,
            summary=f"Creating survival preflight from {source_path}" if source_path else "Waiting for cleaning plan",
        )

    def _finish_task(self, task: TaskRecord, result: SurvivalPreflightResult) -> None:
        now = datetime.now(timezone.utc).isoformat()
    ...
    def _write_output(self, project_id: str, source_path: Path, items: list[object]) -> Path:
        output_dir = self._storage_root / "projects" / project_id / "bioinformatics" / "survival"
        output_dir.mkdir(parents=True, exist_ok=True)
        output_path = output_dir / f"geo_survival_preflight_{uuid4().hex[:12]}.json"
        payload = {
            "project_id": project_id,
            "source_path": str(source_path),
