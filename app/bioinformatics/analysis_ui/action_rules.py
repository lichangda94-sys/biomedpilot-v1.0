...
def build_action_rows(
    *,
    ...
    risk_score_calibration_decision_curve_plot_gate: dict[str, Any] | None = None,
    ...
    risk_score_calibration_decision_curve_plot_gate = risk_score_calibration_decision_curve_plot_gate or {}
...
def _risk_score_plot_nomogram_action(gate: dict[str, Any]) -> dict[str, Any]:
    blockers = _list(gate.get("blockers")) or ["b37_risk_score_renderer_activation_required"]
    return _disabled(
        "risk_score_plot_nomogram",
        "Plan risk score plot / nomogram",
...
def _risk_score_advanced_visualization_action(gate: dict[str, Any]) -> dict[str, Any]:
    blockers = _list(gate.get("blockers")) or ["b40_risk_score_advanced_visualization_activation_required"]
    return _disabled(
        "risk_score_advanced_visualization",
        "Plan nomogram / calibration / decision curve",
...
def _risk_score_advanced_preflight_action(gate: dict[str, Any]) -> dict[str, Any]:
    if gate.get("status") == "passed_preflight_only":
        return {
            "action_id": "risk_score_advanced_preflight",
            "label": "Review advanced visualization preflight",
            "state": "passed_preflight_only",
            "button_behavior": "enabled_preflight_review_only_no_artifact",
            "enabled": True,
            "normal_user_visible": True,
            "disabled_reason": "",
            "next_action": "Review validated time horizon, outcome mapping, event count, threshold grid and clinical-boundary acknowledgement; this does not generate nomogram, calibration, decision-curve, report-ready output or clinical interpretation.",
        }
    blockers = _list(gate.get("blockers")) or ["advanced_visualization_preflight_not_passed"]
    return _disabled(
        "risk_score_advanced_preflight",
        "Review advanced visualization preflight",
...
def _risk_score_calibration_decision_curve_input_action(gate: dict[str, Any]) -> dict[str, Any]:
    if gate.get("status") == "ready_for_future_artifact_gate":
        return {
            "action_id": "risk_score_calibration_decision_curve_input",
            "label": "Review calibration / decision curve inputs",
            "state": "ready_for_future_artifact_gate",
            "button_behavior": "enabled_input_review_only_no_artifact",
            "enabled": True,
            "normal_user_visible": True,
            "disabled_reason": "",
            "next_action": "Review real calibration and decision-curve input readiness only; this does not compute calibration statistics, net benefit, plot artifacts, report-ready output or clinical interpretation.",
        }
    blockers = _list(gate.get("blockers")) or ["calibration_decision_curve_input_gate_not_ready"]
    return _disabled(
        "risk_score_calibration_decision_curve_input",
        "Review calibration / decision curve inputs",
        str(gate.get("status") or "blocked_planning_only"),
...
def _risk_score_calibration_decision_curve_plot_action(gate: dict[str, Any]) -> dict[str, Any]:
    if gate.get("status") == "passed":
        return {
            "action_id": "risk_score_calibration_decision_curve_plot",
            "label": "Generate calibration / decision curve plot",
            "state": "enabled_plot_artifact_from_statistics",
            "button_behavior": "enabled_b45_statistics_sourced_svg_plot_only",
            "enabled": True,
            "normal_user_visible": True,
            "disabled_reason": "",
            "next_action": "Generate a controlled SVG calibration or decision-curve plot from B44 statistics tables only; this does not create report-ready output, risk groups, treatment recommendation or clinical interpretation.",
        }
    blockers = _list(gate.get("blockers")) or ["calibration_decision_curve_plot_gate_not_passed"]
    return _disabled(
        "risk_score_calibration_decision_curve_plot",
        "Generate calibration / decision curve plot",
        str(gate.get("status") or "blocked"),
        "; ".join(dict.fromkeys([*blockers, "B45 requires B44 statistics tables; no plot is created from preflight/config alone, and no report-ready package or clinical conclusion is generated."])),
        "Generate B44 statistics tables first, then create a controlled B45 SVG plot artifact.",
    )
