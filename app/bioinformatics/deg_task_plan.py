...
def build_deg_preflight(
    ...
    *,
    ...
    imported_deg_assets = [asset for asset in assets if str(asset.get("asset_type") or "") in {"differential_result_table", "deg_result_table"}]
    ...
    blockers: list[str] = []
    warnings: list[str] = []
    package_blockers = [str(item) for item in (deg_package or {}).get("blockers", []) or []]
    if comparison_config is not None:
        package_blockers = [item for item in package_blockers if item not in {"missing_sample_metadata", "missing_sample_metadata_asset"}]
    if package_blockers:
    ...
    manifest = {
        ...
        "input_summary": {
            "has_count_matrix": _has_asset_type(expression_assets, "raw_count_matrix") or _has_asset_type(expression_assets, "count_matrix"),
...
def _preferred_expression_asset(assets: list[dict[str, object]]) -> dict[str, object] | None:
    priority = {"raw_count_matrix": 0, "count_matrix": 0, "expression_matrix": 1, "normalized_expression_matrix": 2}
...
def _recognition_has_imported_deg(recognition: dict[str, object] | None) -> bool:
    ...
    for item in recognition.get("files", []) or []:
        ...
        for asset in item.get("detected_assets", []) or []:
            if isinstance(asset, dict) and str(asset.get("asset_type") or asset.get("role") or "") in {"differential_result_table", "deg_result_table"}:
                return True
    return False
