...
FEATURE_REGISTRY: tuple[FeatureAvailability, ...] = (
    ...
    FeatureAvailability("meta_analysis", "meta-extraction", "Extraction", FeatureAvailabilityStatus.TESTING, "读取 Screening 队列并为 included 文献生成数据提取池；支持 testing 结构化 ExtractionRecord 表单保存、校验和 CSV 导出，并新增 prevalence、correlation、diagnostic basic outcome 数据结构。", "继续完善 advanced method 表单体验和质量控制。", "app/meta_analysis/legacy/extraction"),
    FeatureAvailability("meta_analysis", "meta-analysis", "Analysis", FeatureAvailabilityStatus.TESTING, "读取 Extraction 输出并执行 Analysis 预检，可构建 analysis-ready dataset，支持基础 testing pooled effect、prevalence/correlation/diagnostic basic 统计、subgroup、leave-one-out、publication bias basic、forest/funnel plot PNG 和 result table CSV；当前不是生产级正式 Meta 统计，network meta 仅为 not implemented placeholder。", "继续完善 advanced analysis UI、统计校验和正式报告解释。", "app/meta_analysis/legacy/analysis"),
    FeatureAvailability("meta_analysis", "meta-reporting", "Reporting", FeatureAvailabilityStatus.TESTING, "保留 Analysis 预检测试版 Markdown 摘要，支持 testing PRISMA 数字摘要、formal Markdown/HTML/DOCX report 雏形、advanced method summary、supplementary exports、figure package、project snapshot 和复现包；PDF 正式报告仍未开放。", "继续完善 publication export、PDF 策略和投稿级模板。", "app/meta_analysis/legacy/reporting"),
    FeatureAvailability("meta_analysis", "meta-ai-assisted-review", "AI-assisted Review", FeatureAvailabilityStatus.TESTING, "支持 testing AI suggestion queue；AI 只生成候选建议，必须人工 accept / reject / edit，并且 accepted 后还需要明确 apply。AI 不会直接覆盖正式 screening、extraction、analysis 或 report 数据。", "继续接入受控本地/外部 AI provider，并完善人工审核 UX。"),
