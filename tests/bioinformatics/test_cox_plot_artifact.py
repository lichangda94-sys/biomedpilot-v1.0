...
def test_cox_forest_plot_artifact_is_spec_only_and_registered(tmp_path) -> None:
    result = _run(tmp_path)

    plot = create_cox_forest_plot_artifact(str(tmp_path), result["result_id"])

    assert plot["plot_type"] == "cox_forest_plot"
...
def test_cox_plot_blocks_preflight_testing_source(tmp_path) -> None:
    ...
    plot = create_cox_forest_plot_artifact(str(tmp_path), "preflight")

    assert "cox_forest_plot_requires_formal_computed_result_source" in plot["blockers"]
