...
from app.bioinformatics.deg_engine.r_deseq2_planning import save_r_deseq2_parameter_confirmation
from app.bioinformatics.deg_engine.r_deseq2_runtime import run_r_deseq2_rscript_execution
from app.bioinformatics.deg_engine.r_limma_design import save_r_limma_design_config


def test_deseq2_design_confirmation_flow_enables_gated_rscript_execution(tmp_path: Path, monkeypatch) -> None:
    _write_standardized_state(tmp_path)
    monkeypatch.setattr(
        "app.bioinformatics.analysis_ui.state.detect_r_deseq2_runtime_capabilities",
        lambda timeout_seconds=5: _runtime_detection(),
    )

    initial = build_analysis_center_state(tmp_path)
    assert _action(initial, "r_deseq2_parameter_confirmation")["enabled"] is False
    assert _action(initial, "formal_deg_deseq2_rscript")["enabled"] is False
    assert "multi_factor_design_config_missing" in _action(initial, "formal_deg_deseq2_rscript")["disabled_reason"]
    ...
    assert _action(with_design, "r_deseq2_parameter_confirmation")["enabled"] is True
    assert _action(with_design, "r_deseq2_parameter_confirmation")["state"] == "requires_user_confirmation"
    assert _action(with_design, "formal_deg_deseq2_rscript")["enabled"] is False
    assert "r_deseq2_parameter_confirmation_missing" in _action(with_design, "formal_deg_deseq2_rscript")["disabled_reason"]

    deseq2_plan = with_design["r_count_model_plans"]["plans"]["deseq2"]
    confirmation = save_r_deseq2_parameter_confirmation(
        ...
        multi_factor_preflight=deseq2_plan["preflight"],
        dependency_snapshot=deseq2_plan["runtime_gate"]["dependency_snapshot"],
    )
    assert confirmation["status"] == "confirmed"

    confirmed = build_analysis_center_state(tmp_path)
    assert _action(confirmed, "r_deseq2_parameter_confirmation")["state"] == "confirmed"
    action = _action(confirmed, "formal_deg_deseq2_rscript")
    assert action["enabled"] is True
    assert action["button_behavior"] == "enabled_b25_11_audited_deseq2_rscript_only"
    capability = next(row for row in confirmed["analysis_capability_map"]["rows"] if row["capability_id"] == "deg_deseq2")
    ...
    current_plan = confirmed["r_count_model_plans"]["plans"]["deseq2"]
    ...
    result = run_r_deseq2_rscript_execution(
        ...
        source_dataset_id="deseq2-ui-activation-fixture",
    ...
    assert result["result_index_entry"]["engine_name"] == "r_deseq2_rscript_adapter"
    assert {artifact["artifact_type"] for artifact in result["result_index_entry"]["output_artifacts"]} == {"deg_result_table", "deseq2_result_table"}
...
def _runtime_detection() -> dict[str, object]:
    ...
    dependency_snapshot = {
        ...
        "dependencies": {
            ...
            "DESeq2": {"installed": True, "version": "1.46.0"},
        },
        "blockers": [],
    }
    return {
...
def _fake_rscript(tmp_path: Path) -> Path:
    ...
    path.write_text(
        ...
        "    'ENSG000002\\tGENE2\\t50.0\\t-1.8\\t0.5\\t-3.6\\t0.004\\t0.02\\n',\n"
        "    encoding='utf-8',\n"
        ")\n",
        encoding="utf-8",
    )
