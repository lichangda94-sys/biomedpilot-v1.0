...
from app.bioinformatics.deg_engine import check_multifactor_r_backend, run_controlled_multifactor_edger_fixture
...
def test_multifactor_edger_backend_detects_external_r_runtime() -> None:
    snapshot = check_multifactor_r_backend("edgeR")
    ...
    if snapshot["status"] == "passed":
        assert snapshot["r_backend"]["packages"]["edgeR"]["available"] is True
    else:
...
def test_controlled_multifactor_edger_fixture_registers_formal_result(tmp_path: Path) -> None:
    ...
    assert entry["engine_name"] == "r_edger_multifactor"
...
def test_multifactor_edger_blocks_non_count_value_type(tmp_path: Path) -> None:
    result = run_controlled_multifactor_edger_fixture(tmp_path, value_type="log_expression")
