...
def test_analysis_center_state_comes_from_b8_contracts_and_has_no_side_effects(tmp_path: Path) -> None:
    ...
    assert state["developer_diagnostics"]["frontend_consumption_matrix"]["pending_detail_view_count"] == 2
    assert "formal_deg_review_panel" not in state["developer_diagnostics"]["frontend_consumption_matrix"]["pending_detail_view_ids"]
    assert state["developer_diagnostics"]["frontend_consumption_matrix"]["migrated_detail_view_ids"] == ["formal_deg_review_panel"]
    assert state["developer_diagnostics"]["frontend_consumption_rows"] == state["frontend_consumption_rows"]
    assert "pending_detail_views=2" in frontend_consumption_text
    assert "migrated_detail_views=1" in frontend_consumption_text
