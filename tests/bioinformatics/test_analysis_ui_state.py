...
def test_analysis_center_state_comes_from_b8_contracts_and_has_no_side_effects(tmp_path: Path) -> None:
    ...
    assert state["developer_diagnostics"]["survival_clinical_state"]["risk_score_calibration_decision_curve_plot_gate"]["report_ready_eligible"] is False
    ...
    assert "Risk score advanced visualization preflight" in survival_rows_text
    assert "formal_risk_score_result_not_found" in survival_rows_text
    assert "Risk score advanced visualization artifact" in survival_rows_text
    ...
    assert "B45 Risk score calibration / decision curve plot artifact gate" in survival_diagnostics_text
