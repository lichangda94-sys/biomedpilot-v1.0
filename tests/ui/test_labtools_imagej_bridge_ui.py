...
def test_labtools_home_opens_calculator_without_exposing_imagej_as_top_level(qapp) -> None:
    ...
    calculator_button = next(
        item
        for item in widget.findChildren(QPushButton, "labtoolsEntryButton")
        if item.property("pageKey") == "general_calculators"
    )
    ...
    widget.show_home()
    assert widget.findChild(QPushButton, "labToolsExternalEngineSettingsButton") is None
    assert "ImageJ 本地后端状态" not in _visible_text(widget._stack.currentWidget())
