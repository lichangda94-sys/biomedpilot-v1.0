...
def test_analysis_architecture_gate_script_allows_current_partial_state_without_full_ready(tmp_path: Path) -> None:
    ...
    assert payload["frontend_consumption_matrix"]["pending_detail_view_count"] == 2
    assert payload["frontend_consumption_matrix"]["pending_detail_view_ids"] == [
        "formal_deg_plot_report_controls",
        "immune_tme_scoring_page",
    ]
    assert payload["frontend_consumption_matrix"]["migrated_detail_view_count"] == 1
    assert payload["frontend_consumption_matrix"]["migrated_detail_view_ids"] == ["formal_deg_review_panel"]
    ...
    assert frontend_rows["detailed_result_views_migration"]["pending_detail_view_count"] == 2
    assert frontend_rows["detailed_result_views_migration"]["migrated_detail_view_ids"] == ["formal_deg_review_panel"]
