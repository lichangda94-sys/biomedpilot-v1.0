...
ImageJ/Fiji status consumer note：LabTools 不再定义独立 ImageJ/Fiji 配置 schema。ImageJ/Fiji 本地工具状态由共享 `app.shared.local_engines` 的 `biomedpilot_local_engine_config.v1` 承担；LabTools 只消费状态并在图像相关入口显示上下文提示。
...
| `biomedpilot_local_engine_config.v1` | 共享本地工具配置，包含 ImageJ/Fiji 状态 | 共享 local engine 层在用户配置或验证时写入 | 外部后端可用性配置，不是 LabTools 分析结果 | 不建议公开；可能包含本机可执行文件路径 | 保存 configured path/endpoint 和 last_status；LabTools 只读取状态 |
...
| `ProteinLoadingResult` JSON-compatible dict | Western Blot 蛋白上样体系计算结果 | 当前不自动写盘 | 上样体系辅助计算草稿 | 不建议作为正式记录公开 | 不包含系统路径 |
| `BcaAnalysisResult` JSON-compatible dict | BCA 蛋白浓度测定辅助计算结果 | 当前不自动写盘 | BCA 测定辅助计算草稿 | 不建议作为正式记录公开 | 不包含系统路径；用户备注可能含内部样本信息 |
...
## 3A. Shared `biomedpilot_local_engine_config.v1` for ImageJ/Fiji

- Producer：shared `app.shared.local_engines` layer, specifically `LocalEngineConfigStore` and `ImageJFijiBridge`.
- LabTools consumer：`read_shared_imagej_fiji_status()` and contextual status panels in image-related UI.
- Main fields：`schema_version`、`engine_id`、`configured_path_or_endpoint`、`last_status`。
- `last_status` fields：`engine_id`、`engine_name`、`engine_type`、`configured_path_or_endpoint`、`detected_version`、`recommended_version`、`status`、`last_check_at`、`last_error`、`smoke_test_result`、`install_guide_url_or_text`。
- Status values：`not_configured`、`configured_unverified`、`available`、`failed`、`unsupported_version`。
- User semantics：只表示本机外部 Fiji/ImageJ local engine 可用性；不表示任何 LabTools 图像分析 workflow 已经可用。
- Local path handling：可能保存本机 executable/app 路径；不应公开分享。
- Boundary：LabTools 不拥有全局 local-engine schema；不自动下载、不静默安装、不打包 Fiji/ImageJ；不做 WB/gel grayscale、fluorescence 自动分析、wound 自动分析、cell counting、自动 ROI 或批处理。
...
- Producer：`calculate_protein_loading()`。
- Current persistence：内存结构 / JSON-compatible dict；当前没有用户确认写盘流程。
- Main fields：settings、samples、per-sample sample/loading buffer/water volumes、overage、totals、warnings、reducing agent notice、review notice。
- Semantics：Western Blot 上样体系辅助计算草稿，使用前需人工核对蛋白浓度测定方法、样本稀释状态、loading buffer 倍数、还原剂要求、加热变性条件和实验室 SOP。
- Boundary：不做 WB/gel grayscale、条带 ROI、背景扣除、target/loading control ratio、实验成功判断或正式报告结论。

### `BcaAnalysisResult`

- Producer：`analyze_bca_assay()`。
- Current persistence：内存结构 / JSON-compatible dict；当前没有用户确认写盘流程、没有 plate layout 模板保存、没有 XLSX 导出。
- Main fields：blank subtraction status、blank mean OD、Plate Raw Data rows、Standard Curve rows、Sample Results rows、linear fit result、warnings、review notice。
- Semantics：BCA 蛋白浓度测定辅助计算草稿；线性拟合、CV%、R²、范围和 blank 扣除结果均需结合试剂盒说明书、标准曲线质量、重复孔一致性和实验室 SOP 人工核对。
- Boundary：只做 BCA v1 线性拟合；不做 Bradford、NanoDrop、ELISA 标准曲线、4PL、自动离群孔删除、临床诊断或正式报告结论。
...
- Shared local engine config：用户配置或验证 ImageJ/Fiji 时由共享 local engine 层写入；LabTools 只读取状态。
