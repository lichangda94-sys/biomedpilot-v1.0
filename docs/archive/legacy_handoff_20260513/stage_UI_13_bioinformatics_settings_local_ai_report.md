...
- 不让本地 AI 参与统计分析结论。
- 不实现真实订阅或账号权限。
- Ollama/local LLM 属于本地环境能力，不消耗平台 AI credits，不受云端会员或付费 API key 权益限制。
- 云端付费 AI、账号、积分、会员、API key 和用量/费用记录必须另走 cloud AI 设计，不能混入本地模型检测。
- 后续本地模型入口应按功能触发；普通用户不应先经过复杂的主界面本地模型配置中心，高级排障可放在 `设置 > 本地工具与模型`。
