...
| Standard worker migration evidence validator | Present; `analysis/schemas/output/standard_worker_migration_evidence.schema.json` and `validate_standard_worker_migration_evidence()` block migration completion claims unless a selected module has a schema-valid full-mode evidence payload, full-mode standard package, standard R worker boundary, task-center invocation, result-index registration, formal semantics preservation, and UI standard-package consumption evidence. Mock, lite, and legacy sidecar packages do not pass. |
