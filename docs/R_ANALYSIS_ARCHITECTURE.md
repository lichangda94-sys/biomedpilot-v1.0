...
The same matrix reports standard-worker lite replacement candidates without treating them as migration completion evidence. The current lite candidate is `immune_infiltration`, where the existing sidecar is testing/lite scoped and a task-bridge standard R-worker lite path is already covered. Correlation has moved to the task bridge standard R-worker lite path and is no longer a legacy sidecar producer. Formal/controlled sidecars for DEG, enrichment, and survival are not marked as lite replacement candidates even though lite paths exist, because full migration requires formal standard-worker evidence with environment/resource locks.
...
| Legacy sidecar invocation diagnostics | Present for current formal DEG, multifactor DEG, controlled enrichment, survival/KM/Cox, and immune scoring sidecar standard packages; explicitly labeled as non-isolated legacy adapter outputs. Correlation no longer uses this sidecar path for lite execution. |
