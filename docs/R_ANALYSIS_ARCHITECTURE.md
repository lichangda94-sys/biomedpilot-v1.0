...
The architecture status exposes `frontend_consumption_matrix` for RARCH-08. It verifies the standard package catalog source policy, package detail artifact policy, Analysis Center state exposure, and Results Browser standard package provenance/manifest/input/artifact tables. The matrix is currently `partial`: catalog and Results Browser consumption are standard-package based, but module-specific detailed result views still need standard-package-only migration before the frontend boundary can be called complete. The partial row now includes `pending_detail_view_ids` for the known transitional consumers: formal DEG review, formal DEG plot/report controls, and immune/TME scoring.
