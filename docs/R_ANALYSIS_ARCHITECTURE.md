...
| Architecture gate script | Present; `scripts/analysis_architecture_gate.py` emits a schema-checked read-only JSON gate for P0/contract validity, 20 requirement rows, blocked environment/resource visibility, and per-module migration visibility; it can optionally require full readiness. |
