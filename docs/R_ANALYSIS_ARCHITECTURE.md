...
For external environment/resource handoff, the gate can also write `--evidence-template-output <path>`. This JSON package contains environment lock templates, resource lock templates, standard-worker migration evidence templates, registry paths, current blockers, remediation scope, and template counts, and it self-checks against `analysis/schemas/output/evidence_template_package.schema.json`. The package-level self-check now also requires environment templates to carry `renv_lock_content` and resource templates to carry `cache_content`, so external handoff files expose the same non-empty package/resource evidence expectations as the readiness validators. It is a template export only: it does not register evidence, restore full environments, lock resources, execute workers, install packages, or download resources.
