...
| Environment registry | Present; module manifests are tested against `analysis/registry/analysis_environments.json`, Dockerfile paths, renv lock paths, heavy-dependency policy, and allowed-module lists. A runtime validator reports structural status separately from full readiness and keeps full mode blocked while full locks remain scaffold-only. |
