...
`scripts/analysis_architecture_gate.py` provides a command-line wrapper for CI and ReleaseBuild preflight. It emits a `biomedpilot.analysis.architecture_gate_report.v1` JSON payload containing the architecture status, full activation gate, standard-worker migration summary, and remediation queue. The default gate is a P0/contract-validity guard and therefore exits zero for the current `partial_with_p1_gaps` state; `--require-full-ready` exits nonzero until full mode is actually eligible.
...
| Architecture gate script | Present; `scripts/analysis_architecture_gate.py` emits a read-only JSON gate for P0/contract validity and can optionally require full readiness. |
