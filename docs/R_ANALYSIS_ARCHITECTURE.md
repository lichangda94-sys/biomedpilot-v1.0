...
The architecture remediation queue mirrors that module-level migration scope. The `migrate_formal_algorithms_to_isolated_standard_worker` item carries `module_scope` with expected, passed, blocked, and missing evidence modules, allowing a future migration to select one module and register real evidence without relaxing the full activation gate.
