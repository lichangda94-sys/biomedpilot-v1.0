...
The architecture status exposes `standard_worker_entrypoint_matrix` for RARCH-03. It verifies the shared `analysis/runners/run_module.R` CLI contract, standard package output contract, lite module dispatch coverage, main-backend `run_standard_r_worker()` invocation path, and the absence of runtime install or resource acquisition commands in the standard worker. The matrix is currently `partial`: the shared entrypoint and lite dispatch contracts pass, but every formal module remains pending until scoped full-mode migration evidence proves task-center registered isolated standard-worker execution. Analysis Center and the Markdown architecture gate report expose the same rows so the standard entrypoint can be audited without treating lite readiness as full/formal migration evidence.
