...
The same matrix reports standard-worker lite replacement candidates without treating them as migration completion evidence. Current lite candidates are `correlation` and `immune_infiltration`, where the existing sidecar is testing/lite scoped and a task-bridge standard R-worker lite path is already covered. Formal/controlled sidecars for DEG, enrichment, and survival are not marked as lite replacement candidates even though lite paths exist, because full migration requires formal standard-worker evidence with environment/resource locks.
