...
Restored environment locks must also provide schema-valid evidence using `analysis/schemas/output/environment_lock_evidence.schema.json`. `validate_analysis_environment_lock_evidence()` requires the environment id, restored/locked/active status, R version, Bioconductor version, SHA-256 package lock hash, Dockerfile, renv lock, allowed modules, evidence files, and explicit forbidden runtime package/resource download policies. A future renv lock that is marked `restored` without matching evidence remains blocked.
