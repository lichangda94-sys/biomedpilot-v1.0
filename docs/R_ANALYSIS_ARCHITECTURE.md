...
The same matrix reports standard-worker lite replacement candidates without treating them as migration completion evidence. There are currently no remaining lite replacement candidates: immune infiltration and correlation have both moved to the task bridge standard R-worker lite path and are no longer legacy sidecar producers. Formal/controlled sidecars for DEG, enrichment, and survival are not marked as lite replacement candidates even though lite paths exist, because full migration requires formal standard-worker evidence with environment/resource locks.
...
| Legacy sidecar invocation diagnostics | Present for current formal DEG, multifactor DEG, controlled enrichment, and survival/KM/Cox sidecar standard packages; explicitly labeled as non-isolated legacy adapter outputs. Immune scoring and correlation no longer use this sidecar path for lite execution. |
