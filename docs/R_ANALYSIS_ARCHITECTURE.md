...
The standard package catalog is a review and diagnostics surface, not a source of UI execution readiness by itself. Catalog rows expose `ui_execution_eligible`, `migration_evidence_eligible`, `execution_readiness_policy`, `migration_evidence_policy`, and `policy_blockers`. Legacy service-adapter sidecars are kept visible for audit, but they are always marked as review-only and forbidden as standard-worker migration evidence. Standard-worker lite/testing packages are also not migration evidence; a row can only become a candidate after it is both full-mode and `formal_computed_result`, then the migration registry must still validate it.
...
For transitional controlled adapters that still need module-specific R scripts, `app/analysis_runtime/r_worker.py` exposes `run_external_r_command()`. This helper centralizes external R subprocess behavior, timeout handling, blocker payloads, and worker-boundary metadata for adapters that still use the transition boundary, such as multi-factor DEG. It is not the final isolated standard worker interface; adapter-generated packages remain labeled as sidecars until their algorithms are moved behind `analysis/runners/run_module.R` or an equivalent isolated worker service. Controlled enrichment legacy formal execution no longer uses this transition boundary and is blocked until full standard-worker migration evidence, full environment locks, and full resource locks pass.
...
Locked resources must also point to a schema-valid evidence payload using `analysis/schemas/output/resource_lock_evidence.schema.json`. Evidence paths may be declared directly in the resource manifest for repository-local fixtures or registered through `analysis/registry/resource_lock_evidence.json` for externally prepared full resources. That registry declares the authoritative expected full resource/tool scope, and the validator blocks expected-scope drift or unregistered evidence entries. Each evidence payload records resource id, version, source, hash, cache content declaration, license, cache path, approved modules, evidence files, and `runtime_download_allowed=false`. Full resource locks require SHA-256 hash evidence with a 64-character hex value that matches the referenced `cache_path` content plus `cache_content.non_empty=true` in the evidence payload. File cache paths are hashed by file content; directory cache paths use a deterministic hash over sorted relative file paths and contents, and an empty directory is rejected as `analysis_resource_lock_evidence_cache_path_empty` rather than accepted as a locked resource. If `cache_content.file_count` is present, it must match the actual file count under `cache_path`. The special `repository_fixture` hash marker is accepted only for the built-in mock fixture. Missing evidence, malformed evidence, missing cache-content declaration, missing cache/evidence files, empty cache directories, file-count mismatches, manifest/evidence mismatches, or any runtime-download permission block the resource lock. The current repository-local mock fixture has lock evidence; `analysis/registry/resource_lock_evidence.json` declares the expected full resources but has no evidence entries, so full Reactome/MSigDB/spatial/chem resources remain blocked until real external lock evidence exists.
...
`build_analysis_architecture_status()` also exposes `environment_artifact_matrix` for RARCH-12 through RARCH-14. It lists every environment's Dockerfile, renv lock, lock policy status, package count, heavy-dependency policy, allowed modules, and readiness blocker. `app-dev` and `r-bio-core` pass as lightweight/default boundaries, while `r-bio-full`, `r-spatial-full`, `r-chem-full`, and `r-chem-gpu` remain partial because their Docker image builds are not proven and their renv locks are scaffold-only. This matrix performs no Docker build, no renv restore, and no package installation.

`build_analysis_architecture_status()` also exposes `resource_artifact_matrix` for RARCH-15. It lists every declared resource's version, hash, license, cache path, lock evidence path, runtime-download policy, and required modules. The built-in mock fixture remains the only locked resource. Full Reactome/MSigDB/GO/KEGG/OrgDb, spatial, CellChatDB, AutoDock Vina, docking template, GROMACS, and MD force-field resources remain partial until external prelocked cache evidence and registry entries exist. This matrix performs no resource download, no cache preparation, and no evidence auto-registration.

Module manifests are part of the worker-boundary contract. A lite-supported module must declare `runner=analysis/runners/run_module.R` and `worker_backend=rscript` in its own `analysis/modules/<module>/module.json`; the central registry cannot be the only place that supplies the runner. Entrypoint and migration diagnostics also read mode declarations from the module manifest before using registry adapter metadata, so a registry-only change cannot mark lite or full worker readiness without the module manifest carrying the same contract.

The environment validator also emits `environment_lock_evidence_templates` for blocked full environments. Each template records the required evidence shape for the external environment handoff: restored status, R and Bioconductor versions, package-lock hash, non-empty renv lock content, Docker image ref/digest/architecture/build log, Dockerfile, renv lock, allowed modules, evidence files, and explicit forbidden runtime package/resource download policies. These templates are guidance for future registry entries; they do not mark an environment restored.

Restored environment locks must also provide schema-valid evidence using `analysis/schemas/output/environment_lock_evidence.schema.json`. `validate_analysis_environment_lock_evidence()` requires the environment id, restored/locked/active status, R version, Bioconductor version, SHA-256 package lock hash, renv lock content declaration, Docker image build evidence, Dockerfile, renv lock, allowed modules, evidence files, and explicit forbidden runtime package/resource download policies. The declared package-lock hash must match the referenced `renv_lock` file content, the evidence payload must declare `renv_lock_content.policy_status` and `renv_lock_content.packages_non_empty=true`, and the referenced renv lock must itself carry restored/locked/active policy metadata plus a non-empty `Packages` map. If `renv_lock_content.package_count` is present, it must match the actual number of packages in the referenced renv lock. `docker_image` must declare an image ref, SHA-256 digest, architecture, `build_status=built`, and an existing build log. A scaffold-only lock, empty package map, or Dockerfile without image build proof is blocked even when the SHA-256 renv hash matches; a future renv lock that is marked `restored` without matching evidence remains blocked.
...
The same status payload carries `requirement_summary`, `priority_issue_lists`, `p2_issues`, `p3_issues`, and `top_architecture_risks`. These fields use the same priority mapping as the architecture gate report, so Analysis Center, scripts, and handoff consumers can read PASS/WARN/FAIL counts and P0/P1/P2/P3 issue sets from one source of truth.
...
Formal standard-worker migration evidence is centralized in `analysis/registry/standard_worker_migration_evidence.json`. The registry shape is checked by `analysis/schemas/output/standard_worker_migration_evidence_registry.schema.json`, and each evidence entry must pass `analysis/schemas/output/standard_worker_migration_evidence.schema.json`. The registry explicitly declares the authoritative `expected_module_ids` scope for formal migration, and the validator blocks scope drift before reading individual evidence entries. `build_standard_worker_migration_matrix()` reads this registry and will not mark a module as `migrated_to_isolated_standard_worker` unless that module has a schema-valid evidence entry that passes `validate_standard_worker_migration_evidence()`. The current registry intentionally has no evidence entries, so no formal module is marked migrated.

Each standard-worker migration row now also exposes a `migration_evidence_template`, `migration_blockers`, `migration_prerequisite_status`, and `migration_next_action`. The template is a machine-readable checklist for the future registry entry: `mode=full`, a task-center task id, a standard result package directory, standard R worker boundary, task-center invocation, result-index registration, frontend standard-package consumption, and formal semantics preservation. It explicitly forbids mock fixtures, lite testing-level packages, legacy service-adapter sidecars, and module-private output paths as migration evidence. The prerequisite payload classifies lite standard-worker readiness, full-mode registry status, missing registry evidence, formal runtime contract status, legacy-sidecar boundary status, required full environment, required environment/resource locks, and the required task boundary. These fields guide scoped module migration and Analysis UI disabled reasons; they do not mark the current modules as migrated. The matrix also reports `expected_evidence_module_ids`, `passed_evidence_module_ids`, `blocked_evidence_module_ids`, and `missing_evidence_module_ids`, so consumers can show module-level migration gaps without parsing each row; the expected list must match the registry's authoritative expected scope.

Migration evidence is stricter than a valid package-contract diagnostic. `validate_standard_worker_migration_evidence()` reads `result.json`, `provenance.json`, and `logs/worker_invocation.json`; the candidate package must be `status=passed`, `result_semantics=formal_computed_result`, have no result blockers, use `biomedpilot_standard_r_worker`, use `worker_boundary.boundary_type=standard_r_worker`, use `task_system_invocation=task_center_registered`, use `migration_status=standard_worker_contract`, and carry a full analysis environment snapshot whose environment and resource locks are ready. Blocked full packages, testing-level outputs, direct CLI packages, and legacy sidecar packages remain invalid migration evidence.
...
The same gate can also render a nine-section Markdown report with `--markdown-output <path>`. That report is generated from the JSON payload and covers the required human audit sections: current fit, PASS/WARN/FAIL table, top five risks, standard-worker migration evidence coverage, P0/P1/P2/P3 issues, involved files, minimal remediation path, priority files, completed changes, and manual decisions. Markdown rendering is read-only and does not change the activation decision.

The Markdown manual-decision table also carries the remediation scope summary for scoped worker migration, including missing, passed, blocked, and module list counts. This keeps the human report aligned with the JSON remediation queue and Analysis Center diagnostics.

For external environment/resource handoff, the gate can also write `--evidence-template-output <path>`. This JSON package contains environment lock templates, resource lock templates, standard-worker migration evidence templates, module-level full activation diagnostics, registry paths, current blockers, remediation scope, remediation actions, expected evidence scope, and template counts, and it self-checks against `analysis/schemas/output/evidence_template_package.schema.json`. `expected_evidence_scope` mirrors the authoritative full environment ids, full resource/tool ids, and formal module ids that still need registry evidence, so external preparation work cannot silently narrow the handoff to a partial list. `remediation_actions` mirrors the environment, resource, and module next-action ledgers from the architecture remediation queue so handoff consumers can follow the same action ids without parsing the full gate report. `full_activation_module_matrix` and `full_activation_module_rows` mirror the module-level environment/resource/worker blockers from the gate report. The package-level self-check now also requires environment templates to carry `renv_lock_content`, resource templates to carry `cache_content`, expected scope lists to be explicit, remediation actions to be present, and module-level full activation diagnostics to be present. It is a template export only: it does not register evidence, restore full environments, lock resources, execute workers, install packages, or download resources.

Analysis Center renders this as a dedicated `Full analysis activation gate` row in `analysis_architecture_gate_rows`, including disabled blockers for unrestored full environment locks, incomplete full resource locks, and pending standard-worker migration. The standard-worker migration table also includes a `R worker migration evidence coverage` row that lists missing module-level evidence from the migration matrix, keeping lite readiness separate from full/formal isolated-worker migration.

The `restore_full_analysis_environment_locks` remediation item now carries `environment_next_actions` and `environment_action_summary` derived from environment lock evidence templates. Each environment action names the blocked full environment, allowed modules, Dockerfile, renv lock, evidence files, required Docker build proof, package-lock hash requirement, and forbidden runtime install/download evidence sources. These rows are planning evidence only: they do not restore renv locks, build Docker images, register evidence, or mark full environments ready.

The architecture remediation queue mirrors that module-level migration scope. The `migrate_formal_algorithms_to_isolated_standard_worker` item carries `module_scope` with expected, passed, blocked, and missing evidence modules plus `module_next_actions` and `module_action_summary`. Each action row names the module, current blockers, prerequisite statuses, next safe action, and recommended files. This allows a future migration to select one module and register real evidence without relaxing the full activation gate.

The standard-worker migration matrix also carries `adapter_status_counts`, `migration_next_action_counts`, and `migration_blocker_counts`. These summaries group the current controlled Python/R contracts, R-native lite contracts, planned external-tool adapters, and legacy sidecar cases without treating any group as migrated. Analysis Center shows the same grouping as a read-only adapter status summary row.

The architecture status also exposes `full_activation_module_matrix`, a read-only module-level view of full-mode blockers. It combines each module's target full environment, required full resources, and standard-worker migration status, so reviewers can distinguish environment lock blockers from module-specific resource blockers and migration evidence gaps. It does not override the global full activation gate and cannot mark any module full-ready without real environment, resource, and worker evidence.

The architecture status exposes `runtime_acquisition_scan` and `default_dependency_scan` as machine-readable evidence for the P0 default-runtime boundary. The runtime scan searches active `app/`, `analysis/`, `scripts/`, and `config/` sources for package-install and resource-download commands while excluding legacy/cache internals. The dependency scan checks `requirements.txt`, `pyproject.toml`, `docker/Dockerfile.app-dev`, and `renv/renv.app.lock` for heavy full-analysis dependencies. These summaries drive the RARCH-10, RARCH-11, and RARCH-19 pass/fail rows and are also shown in Analysis Center and the Markdown architecture gate report.

The architecture status exposes `module_interface_matrix` as a read-only per-module contract view. It validates that each target module has a module manifest, standard entrypoint, input/output schemas, mock/lite/full declarations, and a mock fixture package that passes the standard result package validator. Analysis Center and the Markdown architecture gate report show the same module interface rows so RARCH-01 through RARCH-07 and RARCH-18 can be audited at module granularity instead of only as aggregate rows.

The architecture status also exposes `module_mode_readiness_matrix` for RARCH-04. It separates contract existence from readiness by listing each module's mock status, lite status, full status, lite environment, full environment, declared full blocker, and next migration action. The matrix is currently `partial`: every target module has mock and lite contracts, while full mode remains blocked by environment/resource/standard-worker prerequisites. This is a diagnostic surface only and does not run or enable full analysis.

The architecture status exposes `standard_worker_entrypoint_matrix` for RARCH-03. It verifies the shared `analysis/runners/run_module.R` CLI contract, standard package output contract, lite module dispatch coverage, main-backend `run_standard_r_worker()` invocation path, and the absence of runtime install or resource acquisition commands in the standard worker. The matrix is currently `passed` for the entrypoint contract. Formal module migration remains visible as pending module ids, but scoped full-mode migration evidence is tracked by the standard-worker migration matrix, P1 remediation, and the task/provenance transition gates rather than by RARCH-03 entrypoint readiness. Analysis Center and the Markdown architecture gate report expose the same rows so the standard entrypoint can be audited without treating lite readiness as full/formal migration evidence.

The architecture status also exposes `external_tool_adapter_matrix` for the chemistry modules. This matrix is a read-only isolation diagnostic for `docking` and `molecular_dynamics`: it verifies that lite mode stays in `r-bio-core`, that lite mode records `external_tool_execution=not_executed_in_lite_mode`, that the full targets remain `r-chem-full` and `r-chem-gpu`, that runtime install is forbidden, that the tools are not default app dependencies, and that AutoDock Vina / GROMACS resource locks are represented as external full-mode requirements. A passed row means the adapter boundary is correctly declared; it does not mean AutoDock Vina or GROMACS have been installed, bundled, executed, or made full-ready.

The architecture status exposes `task_system_boundary_matrix` as a read-only main-backend boundary diagnostic. It verifies that every registered target module has registry-owned result-index task types, the standard `run_analysis_module_task()` bridge, `TaskCenter`, module input schema, standard package schema, result registry, and `worker_invocation.json` schema available. Rows require `task_center_registered` as the task-system invocation for task-bridge packages and keep direct standard-worker CLI artifacts out of UI task-result readiness. Warnings keep current sidecars and pending formal worker migration visible; a passed task-boundary row is not isolated full-worker migration evidence.

The architecture status also exposes `lite_task_bridge_coverage_matrix` as static coverage evidence for the lite worker path. It reads the registered lite module list and the focused task-bridge test source to verify that every lite-supported module is covered by `test_all_registered_lite_modules_run_through_standard_r_worker_package_contract`. That test runs modules through `run_analysis_module_task(..., worker_backend="rscript")`, validates the standard result package, verifies result-index registration and catalog discovery, and confirms `testing_level` semantics plus `report_ready_eligible=false`. The matrix is intentionally read-only during architecture status construction; it does not execute R workers and does not make any full/formal module migrated.

The architecture status exposes `legacy_sidecar_transition_matrix` for RARCH-09. It verifies that legacy service adapter packages must write `worker_backend=legacy_service_adapter`, `task_system_invocation=legacy_service_adapter_direct_call`, and `migration_status=sidecar_only_not_isolated_standard_worker`; that standard-worker packages are blocked from UI task readiness unless they are `task_center_registered`; and that standard-worker migration evidence forbids `legacy_service_adapter_sidecar` sources. The matrix remains `partial` while registry `current_adapter_status` values are transitional inventory rather than completed worker migration evidence.

The same matrix statically scans explicit `allow_legacy_sidecar_execution=True` usage. The override is allowlisted only for tests, `app/bioinformatics/deg_engine/runtime_validation.py`, and `scripts/releasebuild_formal_deg_gate.py`; normal app/UI paths are not allowed to opt into legacy sidecar execution.

The source sidecar producer inventory also checks the default gate on each producer entrypoint. DEG two-group, DEG multi-factor, KM/log-rank, and Cox univariate sidecar writers remain transitional, but the inventory now blocks if their entrypoint no longer calls `legacy_sidecar_execution_gate()`.

The same matrix reports standard-worker lite replacement candidates without treating them as migration completion evidence. There are currently no remaining lite replacement candidates: immune infiltration and correlation have both moved to the task bridge standard R-worker lite path, and controlled enrichment legacy formal execution is blocked instead of inventoried as a live sidecar producer. Formal/controlled sidecars for DEG and survival are not marked as lite replacement candidates even though lite paths exist, because full migration requires formal standard-worker evidence with environment/resource locks.

The architecture status exposes `frontend_consumption_matrix` for RARCH-08. It verifies the standard package catalog source policy, package detail artifact policy, Analysis Center state exposure, and Results Browser standard package provenance/manifest/input/artifact tables. The matrix is currently `partial`: catalog and Results Browser consumption are standard-package based, and the formal DEG review panel now requires a result-index registered `standard_result_package` and reads the DEG table from the package-local artifact manifest. Remaining pending detailed consumers are formal DEG plot/report controls and immune/TME scoring.

The architecture status exposes `reproducibility_provenance_matrix` for RARCH-16. It verifies that `provenance.json` schema requires `input_hash`, `parameter_hash`, `random_seed`, engine name/version, runtime R/Bioconductor/package/external-tool versions, and command; that the standard package validator blocks incomplete passed provenance; and that both the Python task bridge and standard R worker write the required fields. The matrix remains `partial` because legacy service adapter sidecars are transitional provenance only and do not count as isolated standard-worker migration evidence.

The `lock_full_analysis_resources` remediation item now carries `resource_next_actions` and `resource_action_summary` derived from resource lock evidence templates. Each resource action names the blocked resource/tool, required modules, cache path, evidence files, required hash/cache evidence, forbidden evidence sources, and next action `register_schema_valid_prelocked_resource_evidence`. These rows are planning evidence only: they do not download resources, register locks, or mark full resources ready.

Analysis Center also exposes `analysis_resource_gate_rows` as a dedicated resource-lock diagnostic. The rows show the `analysis/resources/manifest.json` structural status, the resource lock evidence registry status, missing full-resource evidence ids, locked fixture resources, and blocked full resources. This keeps Reactome/MSigDB/GO/KEGG/spatial/chem resource blockers visible independently from environment blockers and remediation rows, but it does not register resource evidence or make full mode ready.

Univariate and multivariate clinical association registry status now matches their existing lite standard-worker contracts. Both modules have base R lite fixture execution through `analysis/runners/run_module.R`, emit standard packages, and keep `clinical_conclusion_policy=not_generated`; neither is marked full/formal migrated until full environment/resource locks and registry-owned migration evidence pass.

`app/analysis_runtime/package_catalog.py` builds a read-only catalog from result-index `standard_result_package` artifacts. `build_standard_analysis_package_detail()` reads only a selected standard package directory and returns a stable artifact manifest for declared `tables`, `plots`, `reports`, and package `logs`. `build_analysis_center_state()` exposes this catalog as `standard_analysis_packages`, so Analysis Center can discover standard packages and their artifact paths without reading R package internals or scanning arbitrary output folders. Catalog rows expose `worker_invocation`, `worker_backend`, `worker_invocation_status`, `worker_boundary_type`, `worker_migration_status`, and `artifact_manifest`, so UI diagnostics can use the standard package audit trail and distinguish packages generated by `analysis/runners/run_module.R`, task-bridge fixture copies, bridge-blocked packages, and compatibility sidecars generated by legacy service adapters. A direct `standard_r_worker` CLI package with `task_system_invocation=standard_worker_direct_cli` is blocked in the catalog if it is registered as a UI result, because it is a worker-level diagnostic artifact rather than a main-backend task-center result. Existing module-specific detailed result views still need staged migration.

`app/bioinformatics/deg_engine/result_review.py` now treats standard package consumption as mandatory for formal DEG review. A formal DEG entry without a registered and valid `standard_result_package`, without a package-local `deg_result_table`, or with invalid package semantics is blocked rather than falling back to the legacy `deg_result_table` path. The Results Browser provenance table exposes the standard package path, package validation status, package-local table path, source policy, worker boundary, and migration status for that review.

Controlled enrichment ORA/GSEA legacy formal execution is currently disabled. The compatibility adapter returns a blocked payload with `standard_worker_required=true`, `required_worker_boundary=standard_r_worker`, and blockers for missing full standard-worker migration evidence, full environment locks, and prelocked enrichment resources. The base R enrichment lite worker remains available for testing-level standard packages, but legacy controlled ORA/GSEA sidecars no longer execute or register result-index artifacts.

Existing controlled formal DEG executors can write standard result package sidecars only when a developer/test caller explicitly opts into `allow_legacy_sidecar_execution=True`. The two-group Python controlled DEG runner mirrors successful formal result tables and task logs into `analysis/standard_packages/<result_id>/`; multi-factor limma, DESeq2, and edgeR adapters do the same for fixture-proven formal results. The sidecars preserve result tables, task logs, parameter manifests, dependency snapshots, hashes, engine metadata, and `worker_boundary.boundary_type=legacy_service_adapter_sidecar`. Multi-factor sidecars also preserve formula, contrast, covariates, and batch variables. They are registered in result index v2 as `standard_result_package` output artifacts. Normal direct calls are blocked by `legacy_sidecar_execution_gate()` until the DEG module has task-center registered standard-worker migration evidence. Analysis UI normal-user execution controls also require the DEG module's `standard_worker_migration_matrix` row to pass; with current missing full standard-worker evidence, two-group and multi-factor DEG execution buttons remain disabled even if domain gates pass. This is a package-contract migration step only: it does not enable new DEG execution, plot artifacts, report-ready output, clinical interpretation, or complete migration into the isolated standard worker.

Existing controlled KM/log-rank and Cox univariate executors can write standard result package sidecars only when a developer/test caller explicitly opts into `allow_legacy_sidecar_execution=True`. The sidecars preserve result tables, task logs, parameter manifests, dependency snapshots, hashes, engine metadata, `worker_boundary.boundary_type=legacy_service_adapter_sidecar`, and `logs/worker_invocation.json`. They are registered in result index v2 as `standard_result_package` output artifacts, and their worker invocation manifests are registered as `analysis_worker_invocation_manifest` log artifacts. Normal direct calls are blocked by `legacy_sidecar_execution_gate()` until the survival module has task-center registered standard-worker migration evidence. Analysis UI normal-user execution controls also require the survival module's `standard_worker_migration_matrix` row to pass; with current missing full standard-worker evidence, KM/log-rank and Cox univariate buttons remain disabled even if domain gates pass. This is a package-contract migration step only: it does not enable clinical conclusions, risk grouping, report-ready survival/clinical output, plot artifacts, or complete migration into the isolated standard worker.

Existing exploratory immune / TME scoring now routes through the task bridge and standard R-worker lite path. It writes a standard result package and registers it as a `standard_result_package` output artifact in result index v2. The package preserves the score matrix, signature coverage table, sample summary table, scoring manifest, receipt, worker invocation manifest, limitations, and `worker_boundary.boundary_type=standard_r_worker`. It remains `mode=lite` and `result_semantics=testing_level`; it does not enable GSVA, CellChat, Seurat, CIBERSORT, xCell, ESTIMATE, report-ready immune output, clinical interpretation, or full-mode migration.

Existing local expression-correlation runner outputs now run through `run_analysis_module_task(..., worker_backend="rscript")` and register the task-bridge standard R-worker package as a `standard_result_package` output artifact in result index v2. The package preserves the Pearson correlation result table, summary log, limitations, `worker_backend=rscript`, `worker_boundary.boundary_type=standard_r_worker`, and `task_system_invocation=task_center_registered`. It remains `mode=lite` and `result_semantics=testing_level`; it does not enable report-ready output, causal interpretation, clinical interpretation, or full/formal standard-worker migration.
...
| Standard worker migration evidence validator | Present; `analysis/registry/standard_worker_migration_evidence.json`, `analysis/schemas/output/standard_worker_migration_evidence_registry.schema.json`, `analysis/schemas/output/standard_worker_migration_evidence.schema.json`, `validate_standard_worker_migration_evidence_registry()`, and `validate_standard_worker_migration_evidence()` block migration completion claims unless a selected module has a registry-owned schema-valid full-mode evidence payload, explicit `standard_r_worker` boundary requirements, explicit task-center invocation and standard-worker contract requirements, forbidden evidence source declarations, a passed formal standard result package, no result blockers, standard R worker provenance, ready environment/resource lock snapshots, result-index registration, formal semantics preservation, and UI standard-package consumption evidence. Mock, lite, blocked full, direct CLI, module-private outputs, and legacy sidecar packages do not pass. |
| Environment lock evidence gate | Present; `analysis/schemas/output/environment_lock_evidence.schema.json` and `validate_analysis_environment_lock_evidence()` block restored full environment locks unless SHA-256 package-lock hash matches the referenced renv lock file, Docker image ref/digest/architecture/build log, Dockerfile, renv lock, allowed modules, evidence files, R/Bioconductor versions, non-empty package-lock content, restored lock policy, and no-install/no-download policy are schema-valid and registry-aligned. |
...
| Legacy sidecar invocation diagnostics | Present for current formal DEG, multifactor DEG, and survival/KM/Cox sidecar standard packages; explicitly labeled as non-isolated legacy adapter outputs. Controlled enrichment legacy formal execution is blocked instead of producing sidecars. Immune scoring and correlation no longer use this sidecar path for lite execution. |
...
| Standard package catalog | Present; Analysis Center state exposes result-index-derived package summaries, worker invocation diagnostics, and worker-boundary metadata. Catalog rows are accepted only from `standard_result_package` artifacts inside the active project root; an absolute or relative path that resolves outside the project is blocked and not read. Direct standard R worker CLI packages are also blocked as UI task results unless their invocation boundary is `task_center_registered`. |
...
| Controlled enrichment legacy formal execution | Blocked until full standard-worker migration evidence, full R environment locks, and prelocked enrichment resources pass; no legacy ORA/GSEA sidecar is currently produced. |
| Controlled DEG standard package sidecar | Present for successful two-group Python formal DEG and multi-factor limma/DESeq2/edgeR fixture results with `legacy_service_adapter_sidecar` boundary metadata; normal-user UI execution is blocked until DEG standard-worker migration evidence passes; does not enable new execution, plot/report-ready output, clinical interpretation, or complete isolated worker migration. |
| Controlled survival/clinical standard package sidecar | Present for successful KM/log-rank and Cox univariate controlled results with `legacy_service_adapter_sidecar` boundary metadata and indexed worker invocation manifests; normal-user UI execution is blocked until survival standard-worker migration evidence passes; does not enable clinical conclusions, report-ready output, plot artifacts, risk grouping, or complete isolated worker migration. |
| Exploratory immune/TME standard worker package | Present for bulk signature scoring outputs with `standard_r_worker` boundary metadata; remains `testing_level`/`lite` and does not enable GSVA/CellChat/Seurat, report-ready output, or clinical interpretation. |
| Local correlation task-bridge standard package | Present for Pearson expression-correlation runner outputs with `standard_r_worker` boundary metadata; remains `testing_level`/`lite` and does not enable report-ready output or clinical interpretation. |
