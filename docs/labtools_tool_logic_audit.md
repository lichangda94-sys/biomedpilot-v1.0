...
- LabTools consumes shared ImageJ/Fiji local-engine status from `app.shared.local_engines`; it does not own the global local-engine detection/config architecture.
- The shared layer was carried over from Integration commit `5cda2bc` and provides status/config/detection/smoke-test primitives.
...
| `imagej_fiji.status_consumer_v1` | ImageJ/Fiji status consumer | image_backend_status_consumer | implemented | no | no | medium |
...
| `imagej_fiji.status_consumer_v1` | yes | no | no | Keep as shared local-engine status consumer; discuss every concrete macro workflow separately. |
...
| `imagej_fiji.status_consumer_v1` | ImageJ/Fiji status consumer | image_backend_status_consumer | implemented | `app/labtools/imagej_bridge.py`; `app/labtools/ui/imagej_bridge_widgets.py`; `app/shared/local_engines/*` | `test_imagej_bridge.py`; `test_labtools_imagej_bridge_ui.py`; `test_local_engines_imagej_fiji.py` | no | no | shared `EngineStatus` / shared local engine config | enter image-related workflow; view contextual status; choose setup action if needed | status label; setup prompt; install guide text | backend availability context only | manual setup review | missing shared config; not configured; failed validation; unsupported or unknown version | yes | medium | no | no | keep as consumer only; require Tool Logic Card for every macro workflow |
