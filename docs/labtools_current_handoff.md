...
- 当前分支：`dev/labtools`
- 当前最新功能 commit：`be8e5e8ef96c7c5ca39e646dc66ca7b5d04f0741`
...
| LabTools Stage L4A | `be8e5e8ef96c7c5ca39e646dc66ca7b5d04f0741` | 建立图像分析基础框架：图片记录、分析任务、ROI、占位结果、审计记录和图像定量 UI 入口；四类任务草稿已存在，但未实现真实算法，未生成任何图像定量结果。 |
...
- `algorithm_not_available` 占位状态。
- 图片路径校验支持扩展名、路径存在性和文件大小提示。
- 当前不生成 fake 定量结果。
...
文档类任务可按总手册使用较小验证集；涉及 UI、Shell、入口、功能代码或测试文件时，应恢复运行对应 UI / LabTools 测试。
...
- `LabToolsWorkspaceWidget.page_keys()` 当前包含：
  ...
  - `image_analysis`
  - `pending`
- 普通用户界面应继续保持中文友好，不暴露 raw JSON、内部 schema、内部 id 或大量调试细节。
...
- 当前图像分析仅为任务框架和可复核占位结构。
- `ImageAnalysisResult.metrics` 在 L4A 阶段必须为空。
- 当前结果状态必须保持 `algorithm_not_available` 或 `draft`。
- 默认创建的图像分析任务不得是 `completed`。
- UI 必须明确显示：当前阶段仅建立图像分析任务框架，尚未执行自动定量。请勿将占位状态作为实验结果。
...
- 用户自定义配方草稿必须 confirm 后才成为 confirmed user recipe。
- 来源摘录草稿必须先转为用户配方草稿，不能直接成为 confirmed recipe。
- 来源字段已记录：
  - `source_url`
  - `source_title`
  - `source_label`
...
- 图像分析是最高风险增长点：必须避免把占位结果、草稿状态或算法未启用状态表述成实验结果。
- 第一个真实图像算法应尽量小，优先选择边界清晰、人工 ROI 和复核路径明确的任务。
- 任何真实图像算法都必须记录算法名、版本、参数、ROI、输入 provenance、人工复核提示。
- 图像定量 UI 不得默认显示数值结果，除非算法已经实现并通过测试。
...
1. L4B：荧光强度 ROI 分析 MVP，建议优先，因为算法边界相对清晰，适合作为第一个真实图像算法。
...
- `LabTools Stage L4B`：荧光强度 ROI 分析 MVP。
  - 建议范围：本地图片读取、手动 ROI 输入或简单矩形 ROI、像素强度基础统计、背景 ROI 扣除、结果审计和人工复核提示。
  - 必须先明确是否允许使用 Pillow / OpenCV / scikit-image 等依赖；默认不新增大型依赖。
- `LabTools Stage L4C`：划痕实验面积分析 MVP。
  - 建议范围：手动 ROI、阈值预览、面积比例计算、参数记录、人工确认。
  - 不建议从全自动划痕识别开始。
- `LabTools Stage L3B`：受控外部配方检索。
  - 必须单独授权网络访问、来源白名单、下载限制、缓存策略和安全提示。
- `LabTools Stage L5`：本地项目存储。
  - 必须先设计用户选择位置、文件结构、隐私边界、导出格式和清理策略。

## 16. Handoff 结论

LabTools 当前已经具备实验计算器、本地配方库、来源草稿框架和图像分析框架入口。当前最重要的边界是：图像分析尚未执行自动定量，外部来源尚未访问网络，AI Gateway 未调用，用户数据未自动写盘。后续开发应优先保持小步阶段化，并在每个真实算法或真实网络能力启用前单独确认授权、测试计划和安全提示。
