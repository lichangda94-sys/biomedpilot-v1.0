...
- 当前分支：`dev/labtools`
- 当前最近完成阶段：LabTools ImageJ/Fiji Bridge v1，commit 以当前 git log 为准
- 当前进行阶段：下一阶段待定。
...
- 图像后端基础设施：ImageJ/Fiji Bridge v1，用于配置外部 Fiji/ImageJ 路径、执行 macro smoke test 和显示可用性状态；当前不包含具体图像分析 workflow。
...
- ImageJ/Fiji Bridge v1 基础设施已完成：LabTools 负责本机路径配置、常见路径探测、版本检测、macro smoke test、状态显示和配置清除。
- 后续图像分析后端路线继续为 Fiji/ImageJ macro bridge：LabTools 负责参数表单、输入/输出校验、宏模板管理、运行前预览、结果解析、人工复核提示和 no-overwrite 导出策略。
...
- `LabToolsWorkspaceWidget.page_keys()` 当前包含：
  ...
  - `elisa_absorbance`
  - `imagej_fiji_bridge`
- LabTools header 目前提供 `ImageJ/Fiji 后端设置` 入口，用于配置外部 Fiji/ImageJ 后端并运行 smoke test；该入口是 LabTools 局部后端设置，不得升级为总主界面的大型“External Engines”配置中心。
- 后续全局 UX 应按“架构上统一，体验上按需”对齐：ImageJ/Fiji 由具体图像 workflow 进入或触发时检测，缺失时显示上下文 setup prompt；高级排障可集中到 `设置 > 本地工具与模型`。
...
- ImageJ/Fiji：已接入基础 bridge 配置层。当前仅支持外部 Fiji/ImageJ 路径配置、常见路径探测、版本检测、macro smoke test、状态显示和清除配置；不包含任何具体图像定量 workflow。
...
- 后续后端路线：Fiji/ImageJ macro bridge。LabTools 已完成 bridge v1 基础设施；后续具体工具中，LabTools 只负责宏桥接、参数和文件校验、结果解析、人工复核提示、provenance 记录和导出安全；图像定量逻辑应由明确版本的 Fiji/ImageJ macro 承载。
- ImageJ/Fiji 是本地图像分析后端，不是云端 AI、积分能力或会员权益；不得静默下载、静默安装或在用户未进入/触发 workflow 时静默运行 macro。
- Bridge v1 推荐用户使用 Fiji Stable / Java 8；其它版本不强制拒绝，但必须通过本机 smoke test，未能稳定读取版本时记录 `unknown_version`。
...
- 本地工具与模型：LabTools 不得各自重复实现一套独立 local engine / local model 权益逻辑；后续应复用共享 local engine status/config，并保持按功能触发的 UX。
...
3. 下一阶段推荐：为 ImageJ/Fiji Bridge v1 设计第一个具体 macro workflow 的 Tool Logic Card，优先讨论按功能触发的依赖检测、输入文件、macro 参数、输出 CSV/JSON 字段、provenance、人工复核语义和 no-overwrite 导出策略。
...
LabTools 当前已经具备通用计算器、recipe / record draft、来源草稿框架、图像分析框架入口、SDS-PAGE 用户模板批量换算工具、Western Blot Protein Loading v1、BCA Assay v1，以及 ImageJ/Fiji Bridge v1 基础设施。现有两个 legacy/testing 图像 MVP 仍为手动 ROI 荧光强度分析和手动 ROI + 阈值划痕面积估算。顶层入口现已调整为六个大模块，Western Blot 模块已有五个分区；其中 SDS-PAGE 配胶模板工具只基于用户录入模板计算，不内置通用配方、不推导胶浓度、不推荐配方、不生成配置步骤。后续具体图像分析工具应通过用户配置的 Fiji/ImageJ 和 macro bridge 设计；WB 灰度分析、细胞计数、自动 ROI、fluorescence 自动分析、wound 自动分析和批处理仍未实现；划痕结果不得自动解释迁移效果；外部来源尚未访问网络；AI Gateway 未调用；用户数据和结果导出默认不自动写盘。
