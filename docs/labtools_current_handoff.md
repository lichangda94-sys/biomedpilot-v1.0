...
- `LabToolsWorkspaceWidget.page_keys()` 当前包含：
  ...
  - `elisa_absorbance`
  - `imagej_fiji_bridge`
- LabTools header 提供 `ImageJ/Fiji 后端设置` 入口，用于配置外部 Fiji/ImageJ 后端并运行 smoke test。
...
任何启用网络、AI Gateway、本地模型、OpenCV 或 scikit-image 的任务，都必须作为单独阶段明确授权，并重新设计安全边界和测试计划。后续基于 ImageJ/Fiji bridge 开发具体图像工具时，仍必须单独确认 Tool Logic Card，不得顺带开发新定量算法。
...
3. 下一阶段推荐：为 ImageJ/Fiji Bridge v1 设计第一个具体 macro workflow 的 Tool Logic Card，优先讨论输入文件、macro 参数、输出 CSV/JSON 字段、provenance、人工复核语义和 no-overwrite 导出策略。
