...
- 当前分支：`dev/labtools`
- 当前最近完成阶段：LabTools Current Status + ImageJ/Fiji Bridge Planning，commit 以当前 git log 为准
- 当前进行阶段：下一阶段待定。
...
- 下一阶段建议为 `ImageJ/Fiji Bridge v1`。
- 图像分析后端路线改为 Fiji/ImageJ macro bridge：LabTools 负责参数表单、输入/输出校验、宏模板管理、运行前预览、结果解析、人工复核提示和 no-overwrite 导出策略。
...
- ImageJ/Fiji bridge 尚未实现；不得在 UI 或文档中把 Fiji/ImageJ 接入、自动 ROI、细胞计数、WB 灰度或批处理描述为已完成。
...
- 未实现 ImageJ/Fiji 接入；下一阶段推荐实现 Fiji/ImageJ macro bridge v1。
...
- AI Gateway：未调用。
- 本地模型：未调用。
- ImageJ/Fiji：未接入；已确定为后续图像分析后端路线，下一阶段建议实现 Fiji/ImageJ macro bridge v1。
- OpenCV / scikit-image：未接入，且不作为当前推荐后端路线。
- Pillow：已作为 L4B 最小图片读取依赖接入。

任何启用网络、AI Gateway、本地模型、ImageJ/Fiji、OpenCV 或 scikit-image 的任务，都必须作为单独阶段明确授权，并重新设计安全边界和测试计划。ImageJ/Fiji Bridge v1 应只做本地 Fiji/ImageJ macro bridge、宏模板、参数校验、运行/错误反馈和结果解析，不得顺带开发新定量算法。
...
- 后续后端路线：Fiji/ImageJ macro bridge。LabTools 只负责宏桥接、参数和文件校验、结果解析、人工复核提示、provenance 记录和导出安全；图像定量逻辑应由明确版本的 Fiji/ImageJ macro 承载。
...
3. 下一阶段推荐：ImageJ/Fiji Bridge v1，只做本地 Fiji/ImageJ macro bridge 基础设施，不新增图像定量算法。
...
LabTools 当前已经具备通用计算器、recipe / record draft、来源草稿框架、图像分析框架入口、SDS-PAGE 用户模板批量换算工具、Western Blot Protein Loading v1 和 BCA Assay v1，以及两个 legacy/testing 图像 MVP：手动 ROI 荧光强度分析、手动 ROI + 阈值划痕面积估算。顶层入口现已调整为六个大模块，Western Blot 模块已有五个分区；其中 SDS-PAGE 配胶模板工具只基于用户录入模板计算，不内置通用配方、不推导胶浓度、不推荐配方、不生成配置步骤。后续图像分析后端路线改为 Fiji/ImageJ macro bridge；WB 灰度分析、细胞计数、自动 ROI 和批处理仍未实现；划痕结果不得自动解释迁移效果；外部来源尚未访问网络；AI Gateway 未调用；用户数据和结果导出默认不自动写盘。
