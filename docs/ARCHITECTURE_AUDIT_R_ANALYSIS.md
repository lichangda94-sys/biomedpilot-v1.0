...
| Provenance captures versions/hashes/seed/command | WARN | The standard R worker records separate input and parameter hashes plus seed and command; bridge-blocked full packages now record target environment Dockerfile/renv/resource-lock snapshots; full/formal standard package validation blocks missing provenance containers, but package/tool version capture is still incomplete for unmigrated formal/full modules. |
