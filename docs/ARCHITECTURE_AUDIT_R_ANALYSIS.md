...
- The architecture status now carries `legacy_sidecar_transition_matrix` and `legacy_sidecar_transition_rows`. These rows verify the legacy sidecar writer contract, catalog guard against direct standard-worker UI task readiness, standard-worker migration evidence's sidecar prohibition, registry adapter inventory scope, actual source files that still produce legacy sidecars, and current sidecar test coverage. The matrix remains `partial` because DEG, enrichment, survival/clinical, immune/TME scoring, and correlation still have source-level legacy service-adapter sidecar writers; registry `pending` or `planned` statuses alone are no longer treated as sidecar evidence.
- The legacy sidecar transition matrix now also reports `standard_worker_lite_replacement_candidate_module_ids`. The current lite replacement candidates are `correlation` and `immune_infiltration`, because their sidecars are testing/lite scoped and a task-bridge standard R-worker lite path already exists. DEG, enrichment, and survival sidecars also have lite worker paths, but those are not equivalent to their formal/controlled sidecars and remain blocked until full standard-worker migration evidence passes.
...
| Main backend task-system invocation | WARN | A mock/lite/full-blocking bridge now creates `TaskCenter` entries and result-index entries; registry-declared lite modules are covered through the standard R runner, result package validator, result index, and catalog, while registry-declared full modules are blocked before worker execution with standard package provenance and explicit target environment/resource-lock snapshots. The sidecar transition matrix now inventories actual source-level legacy service-adapter producers: DEG, enrichment, survival/clinical, immune/TME scoring, and correlation. These remain transitional diagnostics and still need task-center registered standard-worker migration after full environment/resource locks are ready. |
