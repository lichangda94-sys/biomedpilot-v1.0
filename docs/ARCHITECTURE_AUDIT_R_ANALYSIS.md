...
- The architecture status now carries `legacy_sidecar_transition_matrix` and `legacy_sidecar_transition_rows`. These rows verify the legacy sidecar writer contract, catalog guard against direct standard-worker UI task readiness, standard-worker migration evidence's sidecar prohibition, registry adapter transition scope, and current sidecar test coverage. The matrix remains `partial` because transitional adapter statuses are inventory, not full worker migration evidence.
- The architecture status now carries `frontend_consumption_matrix` and `frontend_consumption_rows`. These rows verify standard package catalog source policy, package detail artifact policy, Analysis Center state exposure, and Results Browser standard package tables while keeping module-specific detailed result views as a visible partial migration warning. Formal DEG review now requires a result-index registered standard result package and package-local DEG table artifact. Remaining pending detailed views are formal DEG plot/report controls and immune/TME scoring.
