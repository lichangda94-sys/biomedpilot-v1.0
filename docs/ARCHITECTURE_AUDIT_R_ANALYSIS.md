...
- These lite packages are `testing_level`; full enrichment, Reactome/MSigDB resources, full survival packages, plot/report-ready export, prognosis, treatment guidance, and clinical interpretation remain disabled.
...
| Mock/lite/full design | WARN | Registry declares all three modes; every module has fixed mock input/output fixtures; enrichment and survival have base R lite fixtures; other lite modes and full modes remain blocked pending migration. |
...
| Main backend task-system invocation | WARN | A mock/lite bridge now creates `TaskCenter` entries and result-index entries; it can explicitly invoke the standard R runner for mock, enrichment-lite, and survival-lite packages. Existing analysis calls still include direct service calls. |
...
| Issue | Evidence | Status after this audit |
| --- | --- | --- |
| No lite/full environment split | No `docker/` or `renv` split existed before scaffold | Partially fixed with scaffold plus enrichment and survival base R lite fixtures; real package locks and builds pending. |
...
- Added the first standard worker lite paths: enrichment base R ORA and survival base R KM/log-rank fixtures producing testing-level standard result packages.
