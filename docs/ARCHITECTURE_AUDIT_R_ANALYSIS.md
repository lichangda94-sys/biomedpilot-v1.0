...
| Worker invocation audit trail | WARN | All standard task-bridge outcomes, direct standard R runner outputs, and current transitional service-adapter sidecar packages now persist `logs/worker_invocation.json`; task-bridge entries register it as `analysis_worker_invocation_manifest`; sidecar manifests remain explicitly labeled as non-isolated legacy adapter diagnostics. |
