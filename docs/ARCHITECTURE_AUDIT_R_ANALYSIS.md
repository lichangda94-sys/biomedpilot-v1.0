...
| Worker invocation manifest was diagnostic-only | Task-bridge packages now require schema-valid `logs/worker_invocation.json`; invalid policy, backend/status, command, blockers, or task-system boundary fields block standard package validation. |
