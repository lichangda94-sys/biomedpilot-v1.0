...
| Unified entrypoint | WARN | Added and tested `analysis/runners/run_module.R` for mock, every registry-declared lite standard package, docking/MD lite command-manifest packages, and blocked unsupported/full standard packages; existing formal real modules do not call it yet. |
| Mock/lite/full design | WARN | Registry declares all three modes; every module has fixed mock input/output fixtures; every module that declares lite support is now covered by a registry-driven bridge test; all full modes remain blocked pending migration. |
...
| Main backend task-system invocation | WARN | A mock/lite bridge now creates `TaskCenter` entries and result-index entries; registry-declared lite modules are covered through the standard R runner, result package validator, result index, and catalog. Existing controlled enrichment and multi-factor DEG sidecars are now labeled as legacy service-adapter sidecars; direct service subprocess calls still remain. |
...
| Issue | Evidence |
| --- | --- |
| Full modules are not yet proven through isolated full-worker environments | Static tests and bridge tests now prove all registered modules can run mock through one interface, and every registry-declared lite module can run through the same R worker, result package validator, result index, and catalog path; all full modules remain blocked. |
