...
- The architecture status now carries `reproducibility_provenance_matrix` and `reproducibility_provenance_rows`. These rows verify the required provenance schema fields, validator blockers for passed packages, task-bridge provenance writer, standard R-worker provenance writer, and worker invocation manifest. The matrix is intentionally still `partial` because legacy service adapter sidecars remain transitional provenance only and are not isolated standard-worker migration evidence.
