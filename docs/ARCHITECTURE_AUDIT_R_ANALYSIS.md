...
- Existing multi-factor DEG adapters use this shared runtime boundary for Rscript commands instead of directly owning `subprocess.run`.
- The controlled enrichment compatibility adapter no longer invokes Rscript at all; legacy formal ORA/GSEA execution is blocked until full standard-worker migration evidence, full environment locks, and full resource locks pass.
- Transitional R adapters no longer import `subprocess` or set `subprocess.run` as their own default runner; optional test runners are passed through to `run_external_r_command()` where the adapter still uses that transition boundary, and the default subprocess owner remains `app/analysis_runtime/r_worker.py`.
...
- `locked` resources must now reference a schema-valid evidence payload under `analysis/resources/locks/`. That payload records resource id, version, source, hash, `cache_content.non_empty=true`, license, cache path, approved modules, evidence files, and `runtime_download_allowed=false`; missing or mismatched evidence blocks the resource lock. Full resources require a SHA-256 hash algorithm and 64-character hex value that matches the referenced `cache_path` content; directory cache paths must contain at least one file, and any provided `cache_content.file_count` must match the actual cache file count. Empty cache directories are blocked instead of being accepted as locked resources. The repository fixture hash marker is allowed only for `mock_fixture_builtin_v1`.
...
- Restored full-environment locks must now provide schema-valid evidence. `validate_analysis_environment_lock_evidence()` requires restored status, R/Bioconductor versions, a SHA-256 package-lock hash that matches the referenced `renv_lock` file content, `renv_lock_content.policy_status`, `renv_lock_content.packages_non_empty=true`, Docker image ref/digest/architecture/build log, Dockerfile, renv lock, allowed modules, evidence files, and explicit no runtime-install/resource-download policies. The referenced renv lock must also have restored/locked/active policy metadata and a non-empty `Packages` map; if `renv_lock_content.package_count` is provided, it must match the actual package count. Scaffold-only or empty package locks are blocked even when their hash matches, and a Dockerfile path alone is not full environment proof.
...
- `validate_analysis_environment_registry()` now emits `environment_lock_evidence_templates` for blocked full environments. These templates describe the future external evidence payloads, including Docker image build proof, without marking the environments restored.
...
- `validate_analysis_resource_manifest()` now emits `resource_lock_evidence_templates` for blocked full resources and external tools. These templates describe the future version/source/hash/license/cache/evidence payloads while keeping runtime downloads and placeholder locks forbidden.
...
- `analysis/schemas/output/standard_worker_migration_evidence_registry.schema.json` validates the authoritative migration registry shape. The registry now declares the expected formal migration module scope, and the validator blocks expected-module scope drift or unexpected evidence entries. The current registry is schema-valid but still contains zero evidence entries, so every formal module remains pending.
- Every standard-worker migration row now exposes `migration_evidence_template`, `migration_blockers`, `migration_prerequisite_status`, and `migration_next_action`. The template records the future full-mode evidence shape required for a real migration claim and explicitly forbids mock fixtures, lite testing-level packages, legacy service-adapter sidecars, and module-private output paths. The prerequisite payload separates missing full-mode registry support, missing registry evidence, formal runtime contract gaps, required full environment/resource locks, and sidecar-only boundaries, so UI/ReleaseBuild gates can show module-specific next actions without claiming migration completion. This is migration planning evidence only; it does not convert current formal algorithms into isolated standard-worker runs.
- `validate_standard_worker_migration_evidence()` now inspects the candidate package payload, not only the evidence flags. A migration claim is blocked unless the package result is `status=passed`, `result_semantics=formal_computed_result`, has no result blockers, is produced by `biomedpilot_standard_r_worker`, and carries a full analysis environment snapshot with ready environment/resource locks. This prevents blocked full packages, testing-level outputs, or sidecar-like packages from becoming migration evidence.
- The standard-worker migration matrix also exposes expected, passed, blocked, and missing evidence module IDs, so CI, ReleaseBuild, and UI diagnostics can show exactly which formal analysis lines still lack isolated-worker migration evidence.
...
- The same script can export an external handoff package with `--evidence-template-output <path>`. The package contains environment lock templates, resource/tool lock templates, standard-worker migration templates, module-level full activation diagnostics, registry paths, blockers, remediation actions, expected evidence scope, and counts, and self-checks against `analysis/schemas/output/evidence_template_package.schema.json`. Its package-level checker also blocks environment templates without `renv_lock_content`, resource templates without `cache_content`, missing or malformed expected scope lists, missing remediation action ledgers, and missing module-level full activation diagnostics, keeping the handoff shape aligned with the lock validators, remediation queue, activation matrix, and migration matrix. It is a template package only and is not accepted as full-readiness evidence by itself.
...
- The `restore_full_analysis_environment_locks` remediation item now carries `environment_next_actions` and `environment_action_summary` derived from environment lock evidence templates. It lists `r-bio-full`, `r-spatial-full`, `r-chem-full`, and `r-chem-gpu` with allowed modules, Dockerfile/renv paths, evidence files, Docker build proof requirements, package-lock hash requirements, and forbidden runtime install/download evidence sources.
- The `lock_full_analysis_resources` remediation item now carries `resource_next_actions` and `resource_action_summary` derived from resource lock evidence templates. It lists blocked Reactome/MSigDB/GO/KEGG/OrgDb, spatial references, CellChatDB, AutoDock Vina, docking templates, GROMACS, and MD force-field resources with required modules, cache/evidence paths, hash/cache requirements, and forbidden runtime-download evidence sources.
- The `migrate_formal_algorithms_to_isolated_standard_worker` remediation item now carries `module_scope`, `module_next_actions`, and `module_action_summary`, so remediation planning can proceed one module at a time without treating lite fixture readiness as full migration. Each module action preserves blocker categories, prerequisite status, next safe action, and recommended files for gate/UI consumers.
- The standard-worker migration matrix now carries adapter-status, migration-next-action, and migration-blocker count summaries. Analysis Center exposes these summaries in an `R worker migration adapter status summary` row so reviewers can see whether modules are currently controlled contracts, R-native lite contracts, planned external-tool adapters, or legacy sidecar cases without opening every module row.
- The architecture gate now emits `full_activation_module_matrix` and `full_activation_module_rows`. These rows combine module target full environment, module-specific full resource locks, and standard-worker migration blockers. Analysis Center and the Markdown gate report expose the same module-level full activation diagnostics while keeping the global full activation gate blocked.
- The external evidence template package now includes the same `full_activation_module_matrix` and `full_activation_module_rows`, so external environment/resource preparation can consume module-level blockers without re-running the full gate report.
- The architecture status now carries `runtime_acquisition_scan` and `default_dependency_scan`. These machine-readable summaries record scanned roots/files, forbidden patterns, hit counts, and policy labels for the no-runtime-install/no-runtime-download and no-heavy-app-dev-dependency P0 guards. Analysis Center exposes the same scan summaries as dedicated gate rows.
- The Markdown architecture gate report now includes a `Runtime Boundary Scan Evidence` table so human audit output shows the runtime install scan, runtime resource download scan, and default app-dev heavy dependency scan without requiring JSON inspection.
- The architecture status now carries `module_interface_matrix` and `module_interface_rows`. These rows validate each target module's manifest, standard entrypoint, input/output schemas, mock/lite/full declarations, and mock fixture package validation status. Analysis Center and the Markdown gate report expose the same module-level interface diagnostics.
- RARCH-17 now uses the target module list plus `module_interface_matrix` as its evidence. The current target modules all pass the shared interface contract, so RARCH-17 is a PASS; remaining formal standard-worker migration is still tracked separately by P1, the standard-worker migration matrix, RARCH-09, and RARCH-16 rather than being duplicated as an interface warning.
- Module interface, entrypoint, and migration matrices now treat each module's `analysis/modules/<module>/module.json` as the authoritative source for mock/lite/full mode declarations. The central registry can supply module listing and adapter metadata, but it cannot mask a module-level manifest drift where lite mode is not directly bound to the shared R worker entrypoint or where full mode is not declared in the module manifest.
- The architecture status now carries `module_mode_readiness_matrix` and `module_mode_readiness_rows`. These rows show each target module's mock, lite, and full mode status, including lite/full environments, declared full blockers, full activation blockers, and migration next action. The current matrix is partial because full mode remains blocked for every target module.
- The architecture status now carries `environment_artifact_matrix` and `environment_artifact_rows`. These rows show each environment's Dockerfile, renv lock, lock policy status, package count, heavy-dependency policy, allowed modules, and readiness blocker. `app-dev` and `r-bio-core` pass as lightweight boundaries; full environments remain partial until Docker image build evidence and restored renv lock evidence are registered.
- The architecture status now carries `resource_artifact_matrix` and `resource_artifact_rows`. These rows show each declared resource's version/hash/license/cache/evidence status, required modules, runtime-download policy, and missing lock evidence. The built-in mock fixture passes; Reactome, MSigDB, GO, KEGG, OrgDb, spatial, CellChatDB, AutoDock Vina, docking template, GROMACS, and MD force-field resources remain partial until schema-valid prelocked resource evidence is registered.
- The architecture status now carries `standard_worker_entrypoint_matrix` and `standard_worker_entrypoint_rows`. These rows verify the shared `analysis/runners/run_module.R` CLI contract, standard package output contract, lite module dispatch coverage, main-backend invocation path, and no-runtime-acquisition rule. The matrix is `passed` for the entrypoint contract; formal module migration remains visible as `formal_pending_module_ids` but is tracked by P1, the standard-worker migration matrix, RARCH-09, and RARCH-16 rather than counted as RARCH-03 entrypoint readiness.
- The architecture status now carries `external_tool_adapter_matrix` and `external_tool_adapter_rows` for docking and molecular dynamics. These rows verify the declared adapter isolation boundary: lite mode remains in `r-bio-core`, lite mode records `not_executed_in_lite_mode`, full mode targets `r-chem-full` or `r-chem-gpu`, runtime install remains forbidden, default app dependency is false, and AutoDock Vina / GROMACS resources are represented as blocked external full-mode locks. This is isolation evidence only; it does not install, bundle, or execute chemistry tools.
- The architecture status now carries `task_system_boundary_matrix` and `task_system_boundary_rows`. These rows verify the main-backend task bridge, TaskCenter boundary, result-index task type mapping, input schema, standard package schema, result registry, and worker invocation schema for every target module. They keep `task_center_registered` as the required task-system invocation for task-bridge packages and keep legacy sidecars/direct CLI outputs marked as transitional diagnostics, not isolated-worker migration evidence.
- The architecture status now carries `lite_task_bridge_coverage_matrix` and `lite_task_bridge_coverage_rows`. This read-only matrix verifies that every lite-supported target module is covered by the registry-driven task-bridge test `test_all_registered_lite_modules_run_through_standard_r_worker_package_contract`, including `run_analysis_module_task(..., worker_backend="rscript")`, standard package validation, result-index registration, standard package catalog discovery, `testing_level` semantics, and `report_ready_eligible=false`. The matrix is static coverage evidence only: it does not execute workers during the architecture snapshot and does not prove full/formal isolated-worker migration.
- The architecture status now carries `legacy_sidecar_transition_matrix` and `legacy_sidecar_transition_rows`. These rows verify the legacy sidecar writer contract, catalog guard against direct standard-worker UI task readiness, standard-worker migration evidence's sidecar prohibition, registry adapter inventory scope, actual source files that still produce legacy sidecars, and current sidecar test coverage. The matrix remains `partial` because DEG and survival/clinical still have source-level legacy service-adapter sidecar writers; registry `pending` or `planned` statuses alone are no longer treated as sidecar evidence.
- The legacy sidecar transition matrix now also reports `standard_worker_lite_replacement_candidate_module_ids`. There are currently no remaining lite replacement candidates: immune infiltration and correlation have both been routed through the task-bridge standard R-worker lite path, and controlled enrichment legacy formal execution is blocked rather than inventoried as a live sidecar producer. DEG and survival sidecars also have lite worker paths, but those are not equivalent to their formal/controlled sidecars and remain blocked until full standard-worker migration evidence passes.
- The architecture status now carries `frontend_consumption_matrix` and `frontend_consumption_rows`. These rows verify standard package catalog source policy, package detail artifact policy, Analysis Center state exposure, Results Browser standard package tables, and migrated detailed result views. Formal DEG review, formal DEG plot/report controls, and immune/TME scoring previews now require result-index registered standard result package artifacts and package-local tables/manifests; the current pending detailed-view list is empty.
- The architecture status now carries `reproducibility_provenance_matrix` and `reproducibility_provenance_rows`. These rows verify the required provenance schema fields, validator blockers for passed packages, task-bridge provenance writer, standard R-worker provenance writer, worker invocation manifest, and the source-level legacy sidecar provenance inventory. The matrix is intentionally still `partial` because DEG and survival/clinical sidecar provenance remains transitional and cannot count as isolated standard-worker migration evidence.
- The architecture status now carries `requirement_summary`, `priority_issue_lists`, `p2_issues`, `p3_issues`, and `top_architecture_risks`. The architecture gate report reuses these fields, keeping UI, scripts, and handoff consumers on one priority-classification source.
- Analysis Center now carries `analysis_resource_gate_rows`. These rows show resource manifest status, resource lock evidence registry status, missing full-resource evidence ids, locked fixture resources, and blocked full resources separately from environment gates and remediation rows. The current full-resource state remains blocked; the rows are visibility only and do not register resource evidence.
- Analysis Center remediation rows now include an `R evidence template handoff preview` row that exposes the exported handoff action counts for full environments, full resources, and module migration. The row is explicitly marked `planning_only_not_readiness_evidence` and points to `scripts/analysis_architecture_gate.py --evidence-template-output <path>`.
- Univariate and multivariate clinical association registry rows now reflect their existing base R lite standard-worker contracts instead of the older `contract_required_before_formal_r_worker` status. They still remain blocked for full/formal migration until scoped full-mode registry support, full environment/resource locks, and schema-valid standard-worker migration evidence exist.
...
- Standard-worker migration evidence now requires schema-level worker-boundary declarations. A future registry entry must explicitly declare `required_worker_boundary=standard_r_worker`, `required_task_system_invocation=task_center_registered`, `required_worker_migration_status=standard_worker_contract`, and forbidden evidence sources for mock fixture packages, lite testing packages, legacy sidecars, and module-private outputs before package validation can complete migration.
- `app/analysis_runtime/package_catalog.py` reads only result-index `standard_result_package` artifacts.
- Standard package catalog rows are now project-root bounded. A result-index `standard_result_package` artifact that resolves outside the active project root is blocked and not read, preventing legacy/module-private output directories from becoming UI-readable package sources through absolute paths.
- Standard package catalog rows now block direct `standard_r_worker` CLI packages when they are registered as UI task results without `task_center_registered`. Direct runner outputs remain useful worker-contract diagnostics, but they do not satisfy the main-backend task-system execution boundary.
...
- The catalog now also exposes row-level execution and migration-evidence policy fields: `ui_execution_eligible`, `migration_evidence_eligible`, `execution_readiness_policy`, `migration_evidence_policy`, and `policy_blockers`. Legacy service-adapter sidecars remain readable for review, but they are explicitly marked `legacy_sidecar_review_only_not_ui_execution_readiness` and `forbidden_legacy_sidecar_not_standard_worker_migration_evidence`. Lite/testing standard-worker packages are also marked non-eligible for migration evidence unless a future package is `mode=full` and `result_semantics=formal_computed_result`.
- Controlled DEG, multi-factor DEG, KM/log-rank, and Cox univariate direct legacy sidecar execution paths now call `legacy_sidecar_execution_gate()` before computing or registering a formal result. The default policy blocks `legacy_service_adapter_sidecar_execution_disabled` until task-center registered standard-worker migration evidence exists; `allow_legacy_sidecar_execution=True` remains only for focused compatibility tests and runtime-validation tooling.
- The legacy sidecar transition matrix now statically scans `allow_legacy_sidecar_execution=True` usage and allows it only under tests plus the controlled runtime/release validation tools. Any normal app/UI path that opts into the override becomes a `legacy_sidecar_override_not_allowlisted` architecture blocker.
- The source sidecar producer inventory now verifies both sides of the transition: writer files must still declare `legacy_service_adapter_sidecar` provenance, and each producer must have an entrypoint gate calling `legacy_sidecar_execution_gate()` with default execution blocked. Analysis Center exposes this as `default_execution_gates=passed:<n>/<n>`.
...
Controlled enrichment ORA/GSEA legacy formal execution is now blocked:

- `app/bioinformatics/enrichment_r_adapter.py` is a compatibility adapter that returns a blocked payload instead of invoking Rscript or writing result-index sidecars.
- The blocked payload requires `standard_worker_required=true`, `required_worker_boundary=standard_r_worker`, `required_task_system_invocation=task_center_registered`, and `required_mode=full`.
- The disabled policy is explicit: full enrichment must wait for full standard-worker migration evidence, restored full R environment locks, and prelocked enrichment resource evidence.
- This preserves the existing lite enrichment standard-worker fixture while preventing legacy controlled ORA/GSEA sidecars from being mistaken for isolated full/formal worker migration.
...
- `app/bioinformatics/immune_infiltration/scoring.py` prepares task-bridge expression/signature inputs and calls `run_analysis_module_task(..., worker_backend="rscript")`.
- The standard R-worker package includes `result.json`, `provenance.json`, `tables/`, `plots/`, `reports/`, and `logs/`.
- The package records `mode=lite`, `result_semantics=testing_level`, and `worker_boundary.boundary_type=standard_r_worker`.
- The current result index registers the package as an `output_artifacts` item with `artifact_type=standard_result_package`.
- This is a lite worker migration step, not a claim that immune/TME scoring full mode, GSVA/CellChat/Seurat, report-ready output, or clinical interpretation are enabled.

Existing local expression correlation now runs through the task bridge and standard R worker lite path:

- `app/bioinformatics/services/correlation_runner.py` adapts local expression matrices into a module input and invokes `run_analysis_module_task(..., worker_backend="rscript")`.
- The standard package includes `result.json`, `provenance.json`, `tables/`, `plots/`, `reports/`, and `logs/`.
- The worker invocation records `mode=lite`, `result_semantics=testing_level`, `worker_backend=rscript`, `worker_boundary.boundary_type=standard_r_worker`, and `task_system_invocation=task_center_registered`.
- The current result index registers the task-bridge standard package as an `output_artifacts` item with `artifact_type=standard_result_package`.
- Correlation is registered in `analysis/registry/analysis_modules.json` and `analysis/modules/correlation/module.json` with fixed mock and lite fixture packages plus full-mode blocking. Its lite runtime is now behind the standard worker; formal/full migration remains blocked until full environment/resource locks and standard-worker migration evidence pass.
...
- This is a lite task-bridge migration step, not a claim that correlation is full/formal migrated or that report-ready output, causal interpretation, or clinical interpretation are enabled.
...
- The immune infiltration lite path writes a standard result package with `tables/immune_score_matrix.tsv`, `tables/signature_gene_coverage.tsv`, `tables/sample_score_summary.tsv`, `plots/lite_immune_heatmap.svg`, `result.json`, `provenance.json`, `reports/README_lite.md`, and `logs/worker.log`.
...
| Every module outputs `result.json` / `provenance.json` | WARN | Mock fixtures prove standard package shape for every registered module; controlled DEG, controlled KM/log-rank, controlled Cox univariate, exploratory immune/TME scoring, and local correlation results now write standard packages. Immune/TME scoring and correlation use the task bridge and standard R worker lite path; the listed controlled formal DEG/survival results remain sidecar packages. Controlled enrichment ORA/GSEA legacy formal execution is blocked until full standard-worker migration evidence passes. Other existing real algorithms still use varied structures. |
...
| Frontend consumes standard package only | PASS/WARN | Analysis Center state now exposes a standard package catalog and standard-package gate rows from result-index artifacts, worker invocation diagnostics, worker-boundary metadata, full-mode environment snapshots, and a standard artifact manifest; package validation blocks declared table/plot/report artifacts that are missing or escape the standard package directories. Formal DEG review, DEG plot/report controls, and immune/TME scoring previews now consume result-index registered standard package artifacts. Remaining caution is that future detailed result views must keep this standard-package-only policy. |
| Main backend task-system invocation | WARN | A mock/lite/full-blocking bridge now creates `TaskCenter` entries and result-index entries; registry-declared lite modules are covered through the standard R runner, result package validator, result index, and catalog, while registry-declared full modules are blocked before worker execution with standard package provenance and explicit target environment/resource-lock snapshots. The sidecar transition matrix now inventories actual source-level legacy service-adapter producers: DEG and survival/clinical. Controlled enrichment legacy formal execution is disabled until standard-worker migration evidence exists. These remain transitional diagnostics and still need task-center registered standard-worker migration after full environment/resource locks are ready. |
...
| Runtime R package installation in user flow | PASS | Machine guard RARCH-10 found no active non-legacy `install.packages`, `BiocManager::install`, `pak::pkg_install`, or `remotes::install_github`. |
| Runtime large resource download in user flow | PASS/WARN | Machine guard RARCH-10 also scans active non-legacy app/analysis/scripts/config surfaces for explicit resource acquisition commands such as `download.file`, R curl downloads, AnnotationHub/BiocFileCache/ExperimentHub access, and shell `wget`/`curl` downloads. Gene-set resource UI/gates block Reactome/GO/KEGG runtime downloads by default and require GMT import or prelocked resources; real full resource locks are still incomplete. |
...
3. **P1: Standard result package is not universal.** Existing modules use result index entries, report packages, and custom paths rather than always producing `result.json` and `provenance.json`; controlled DEG, controlled KM/log-rank, and controlled Cox univariate are now partially remediated with sidecar packages that are explicitly labeled as legacy service-adapter sidecars. Controlled enrichment ORA/GSEA legacy formal execution is now blocked until it is migrated behind the standard worker with full evidence. Immune/TME scoring and local correlation have moved to the task bridge standard R-worker lite path but remain non-formal and non-report-ready.
...
| UI/backend do not yet call standard worker | Existing direct service calls remain | Partially fixed for mock task bridge and standard package catalog/artifact manifest; controlled enrichment legacy formal execution is blocked until it can use the standard worker. Current UI algorithms are not fully migrated. |
...
- Added machine-readable environment/resource lock evidence templates to the validation and architecture gate payloads.
- Added a standalone architecture gate evidence-template export for external environment/resource and module migration handoff, with a dedicated schema contract.
...
- Added `module_mode_readiness_matrix` to expose RARCH-04 mock/lite/full readiness per module while preserving full-mode blockers.
- Added `environment_artifact_matrix` to expose RARCH-12/RARCH-13/RARCH-14 Dockerfile and renv lock artifact readiness per environment while preserving full-mode blockers.
- Added `resource_artifact_matrix` to expose RARCH-15 full resource lock artifact readiness per resource while preserving full-resource blockers.
...
- Added `analysis/schemas/output/standard_worker_migration_evidence_registry.schema.json` so the authoritative migration registry itself is schema-checked before formal worker migration completion can be claimed; the registry now includes an audited `expected_module_ids` scope that must match the module registry.
...
- Added pending detailed result view inventory to the frontend consumption matrix so RARCH-08 names the remaining module-specific UI consumers instead of only reporting a generic warning. Formal DEG review has now moved out of the pending list by requiring a valid standard result package and package-local DEG table artifact.
...
- Added remediation actions to the external evidence template package handoff so exported templates carry the same environment, resource, and module next-action ledgers as the architecture remediation queue.
...
- Added `standard_worker_entrypoint_matrix` to architecture status, the gate JSON/Markdown report, Analysis Center diagnostics, and tests so RARCH-03 is machine-audited separately from formal migration evidence.
...
- Added `requirement_summary`, `priority_issue_lists`, `p2_issues`, `p3_issues`, and `top_architecture_risks` directly to architecture status and made the architecture gate script reuse them.
