...
- Module interface validation now requires every lite-supported module manifest to explicitly declare `runner=analysis/runners/run_module.R` and `worker_backend=rscript`. This prevents the registry from masking a module-level manifest drift where lite mode is not directly bound to the shared R worker entrypoint.
