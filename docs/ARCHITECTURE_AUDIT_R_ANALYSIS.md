...
- Every standard-worker migration row now exposes `migration_evidence_template` and `migration_blockers`. The template records the future full-mode evidence shape required for a real migration claim and explicitly forbids mock fixtures, lite testing-level packages, legacy service-adapter sidecars, and module-private output paths. This is migration planning evidence only; it does not convert current formal algorithms into isolated standard-worker runs.
