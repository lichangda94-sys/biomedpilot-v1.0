...
- RARCH-17 now uses the target module list plus `module_interface_matrix` as its evidence. The current target modules all pass the shared interface contract, so RARCH-17 is a PASS; remaining formal standard-worker migration is still tracked separately by P1, RARCH-03, RARCH-09, and RARCH-16 rather than being duplicated as an interface warning.
