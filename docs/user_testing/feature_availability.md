...
| Meta 分析 | Analysis | 测试中 | 读取 Extraction 输出并执行 Analysis 预检，可构建 analysis-ready dataset，支持基础 testing pooled effect、prevalence/correlation/diagnostic basic 统计、forest plot PNG 和 result table CSV；当前不是生产级正式 Meta 统计，network meta 仅为 not implemented placeholder。 | 接入 subgroup、sensitivity、publication bias 和 funnel plot。 |
