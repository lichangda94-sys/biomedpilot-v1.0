...
Current scan count for this boundary is 924 files. The count includes source, tests, static assets, archived mirrors, docs, and cached artifacts when present. It is an inventory size, not a current-feature count.
...
| TCGA/GTEx facade | `legacy/tcga_gtex/facade.py`, `adapters/*.py`, `processing/*.py`, `download/task_runner.py` | Old TCGA/GTEx search, download, parsing, normalization facade | Current TCGA/GTEx source cards and standardization pages | Current `data_sources/**` and `search_center/**` supersede it | No | Legacy tests only | No | Old locator/task-run contracts | High | rewrite |
...
| Literature CLI/GUI | `legacy/literature_cli.py`, `legacy/literature_gui.py` | Historical literature search/import tools | Meta literature import/search pages are separate current line | No | No | Legacy tests only | No | Old UI/runtime | High | deprecated |
| Legacy sandbox UI | `legacy/ui/module3_sandbox.py` | Old sandbox for asset viewing | No current mainline page | No | No | Legacy tests only | No | Old widgets and state | High | deprecated |
...
| Legacy area | Representative files | Developed material | Current UI page/button mapping | Existing current implementation? | Real run in this audit? | Tests? | Real figure/table/report? | Old state/path dependency | Risk | Recommendation |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Old app/workbench shell | `legacy/app/main_window.py`, `project_shell_widget.py`, `workbench_home_widget.py`, `app_meta/main_window.py`; archive mirror | Standalone dashboard/sidebar/workbench UI | Current Meta pages are under `app/meta_analysis/pages/**` | No | No | Legacy only | No | Old app state and shell | High | deprecated |
...
| Legacy fulltext/OCR/task support | `legacy/fulltext/**`, `legacy/core/task_*.py`, `legacy/scripts/run_task_once.py` | Historical fulltext and task management patterns | Current Meta fulltext/workflow dashboard pages | No direct current use | No | Legacy tests | No | Old task store/filesystem assumptions | High | rewrite/adapter later |
...
| Legacy bias/readiness/profile helpers | `legacy/bias/**`, `legacy/reporting/**`, `legacy/core/dataset_readiness.py`, `legacy/core/profile_*` | Bias/readiness/profile/reporting service patterns | Current Meta quality/reporting pages only conceptually related | No direct current use | No | Legacy tests only | No | Old project/profile store | Medium/high | adapter only after contract mapping |
| Legacy GEO readiness under Meta | `legacy/geo_readiness/**`, `legacy/local_data/**` | Historical GEO/local-data readiness utilities inside Meta legacy tree | Current Bio recognition/standardization, not current Meta runtime | No | No | Legacy tests only | No | Old GEO readiness/local-data assumptions | High | rewrite/reference |
...
| Packaged copies under `dist/**` | Runtime package mirror, not a source-of-truth migration input | Excluded from source migration decisions. |
