...
| --- | --- | --- | --- |
| `dev/bioinformatics` | Bio project flow, Analysis Center, Results Browser, report/export gates, Meta pages, standard analysis package catalog, standard package/input manifest surfacing, full-mode environment blocker snapshots, architecture status gates, remediation queue, migration matrix, and migration evidence validation | Current implementation evidence; not all rerun in this audit | Mainline |
