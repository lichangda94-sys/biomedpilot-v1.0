...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| Bio controlled formal DEG loop | current `dev/bioinformatics` | `app/bioinformatics/deg_engine/formal_runner.py`, `deg_engine/standard_package.py`, `plots/formal_deg.py`, `reports/formal_deg.py`, `workflow_pages.py` | Gated DEG run to review, plot artifact, DEG section package, and standard result package sidecar | Bio Analysis Task Center / Results Browser | Yes | Prior L3 reports/tests; not rerun in this audit | Yes by current test inventory | Table, SVG plot, DEG section package, standard result package sidecar when gates pass | Current B8/B9+ contracts | Medium | reuse |
...
| Full-mode analysis environment blockers | current | `app/analysis_runtime/environment_validation.py`, `analysis/runners/run_module.R`, `analysis/resources/manifest.json`, `tests/test_r_analysis_architecture_contract.py` | Records environment/resource blockers and snapshots when full mode is unavailable | Package catalog / task bridge diagnostics | Yes/current scaffold | Not rerun in this audit | Current focused tests by history | Blocker manifests and logs only, not analysis result | External full resource locks remain policy-gated | Medium | reuse as governance scaffold |
...
| Immune infiltration | current | `immune_infiltration/**`, `immune_infiltration/standard_package.py`, standard lite worker fixture | Bulk immune/TME scoring candidate with testing-level standard package sidecar | Bio immune pages / package catalog scaffold | Yes/current candidate | Not rerun here | Current tests by inventory | Score/report scaffold exists; testing-level standard package sidecar exists | Signature/resource policy | Medium | reuse with proof |
| Expression correlation | current | `services/correlation_runner.py`, `services/correlation_service.py`, `services/correlation_standard_package.py` | Local Pearson correlation service with testing-level standard package sidecar | Bio correlation/result package catalog surfaces | Yes/current candidate | Not rerun here | Current tests by inventory | Correlation table and standard package sidecar, not report-ready | Local expression matrix assumptions | Medium | reuse with proof |
...
| Meta legacy bias/profile/readiness helpers | `app/meta_analysis/legacy/bias/**`, `legacy/reporting/**`, `legacy/core/profile_*` | Bias, readiness, profile, and reporting helper concepts | Meta quality/reporting pages only conceptually | No direct current use | Legacy-only | Legacy tests only | No current figure/table/report claim | Old profile store and task state | Medium/high | adapter/rewrite |
| Meta legacy GEO/local readiness | `app/meta_analysis/legacy/geo_readiness/**`, `legacy/local_data/**` | Historical GEO/local dataset readiness inside Meta legacy tree | Current Bio recognition/standardization, not current Meta result loop | No direct current use | Legacy-only | Legacy tests only | No current result claim | Old GEO local path assumptions | High | reference/rewrite |
