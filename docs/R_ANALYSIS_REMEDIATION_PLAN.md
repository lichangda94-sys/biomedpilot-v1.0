...
Update: module interface validation now requires every lite-supported module manifest to explicitly declare `runner=analysis/runners/run_module.R` and `worker_backend=rscript`. The correlation module manifest is now aligned with the registry, so lite readiness is proven from the module manifest rather than a registry fallback.
...
Update: legacy sidecar transition diagnostics now emit `legacy_sidecar_transition_matrix`. The matrix verifies the legacy sidecar writer contract, catalog task-center guard, migration-evidence sidecar prohibition, registry adapter transition scope, and sidecar test coverage. It remains `partial` while current adapter statuses are transitional inventory; remediation must replace each sidecar/pending adapter with task-center registered standard-worker evidence, not promote sidecar packages.

Update: frontend standard-package consumption diagnostics now emit `frontend_consumption_matrix`. The matrix verifies catalog source policy, package detail artifact policy, Analysis Center state exposure, and Results Browser standard package tables. It remains `partial` because module-specific detailed result views still need standard-package-only migration; this warning is now machine-readable instead of being only prose in RARCH-08.

Update: the RARCH-08 frontend matrix now carries `pending_detail_view_count`, `pending_detail_view_ids`, and `pending_detail_views`. The current pending list is formal DEG review, formal DEG plot/report controls, and immune/TME scoring, each with a next action to consume `build_standard_analysis_package_detail()` and standard package artifact manifests before the frontend boundary can close.

Update: reproducibility provenance diagnostics now emit `reproducibility_provenance_matrix`. The matrix verifies the provenance schema fields, standard package validator blockers, Python task-bridge writer, standard R-worker writer, and worker invocation manifest contract. It remains `partial` because legacy service adapter sidecars are explicitly marked transitional and cannot be used as isolated standard-worker provenance evidence.
...
- Hardened module manifest validation so lite-supported modules must name the shared R worker runner explicitly; `correlation` now declares the same runner as the registry.
...
- Added pending detailed result view inventory to `frontend_consumption_matrix`, including formal DEG review, formal DEG plot/report controls, and immune/TME scoring.
