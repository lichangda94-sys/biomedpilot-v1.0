...
本文件受 `docs/ui/BioMedPilot_UI_Design_Constitution_v2_20260519.md` 约束。若旧阶段报告、旧页面代码或旧设计稿与该 UI 设计宪法冲突，应优先遵守 UI 设计宪法、当前运行时状态和当前测试结果。

当前阶段进入 UI Freeze / UI Consolidation。默认暂停新增主导航、新增主流程页面、大范围高保真视觉和 blocked 页面美化；优先修复 runtime blocker、收敛导航、合并重复页面、标记 shell-only / testing / placeholder / historical asset。
...
测试报告必须分别写明 passed、skipped、failed、skip 原因、failed 原因、当前 HEAD、执行命令，以及页面是否进入真实运行路径。因 import error、dependency missing、runtime unavailable、workspace fallback 或测试条件跳过造成的 skip，不能描述为 UI 流程通过。
